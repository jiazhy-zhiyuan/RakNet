...
int main(void) {
  ...
  bool b = client->Connect(ip, atoi(serverPort), "Rumpelstiltskin",
                           (int)strlen("Rumpelstiltskin"),
                           &pk) == RakNet::CONNECTION_ATTEMPT_STARTED;
#else
  RakNet::ConnectionAttemptResult car = client->Connect(
      ip, atoi(serverPort), "Rumpelstiltskin", (int)strlen("Rumpelstiltskin"));
  ...
  printf("My GUID is %s\n",
         client->GetGuidFromSystemAddress(RakNet::UNASSIGNED_SYSTEM_ADDRESS)
             .ToString());
  puts(
...
// This sleep keeps RakNet responsive
    ...
    if (kbhit()) {
      ...
      if (strcmp(message, "disconnect") == 0) {
        ...
        int index = atoi(str);
        client->CloseConnection(client->GetSystemAddressFromIndex(index),
                                false);
      ...
      if (strcmp(message, "startup") == 0) {
        bool b =
            client->Startup(8, &socketDescriptor, 1) == RakNet::RAKNET_STARTED;
        if (b)
          printf("Started.\n");
        else
      ...
      if (strcmp(message, "ping") == 0) {
        if (client->GetSystemAddressFromIndex(0) !=
      ...
      // the right order
      client->Send(message, (int)strlen(message) + 1, HIGH_PRIORITY,
                   RELIABLE_ORDERED, 0, RakNet::UNASSIGNED_SYSTEM_ADDRESS,
                   true);
    }
    ...
    for (p = client->Receive(); p;
      ...
      switch (packetIdentifier) {
        ...
        case ID_ALREADY_CONNECTED:
          // Connection lost normally
          // printf("ID_ALREADY_CONNECTED with guid %" PRINTF_64_BIT_MODIFIER
          //        "u\n",
          //        p->guid);
          break;
        case ID_INCOMPATIBLE_PROTOCOL_VERSION:
          ...
          break;
        case ID_REMOTE_DISCONNECTION_NOTIFICATION:  // Server telling the
                                                    // clients of another client
                                                    // disconnecting gracefully.
                                                    // You can manually
                                                    // broadcast this in a peer
                                                    // to peer enviroment if you
                                                    // want.
          printf("ID_REMOTE_DISCONNECTION_NOTIFICATION\n");
          break;
        case ID_REMOTE_CONNECTION_LOST:  // Server telling the clients of
                                         // another client disconnecting
                                         // forcefully.  You can manually
                                         // broadcast this in a peer to peer
                                         // enviroment if you want.
          printf("ID_REMOTE_CONNECTION_LOST\n");
          break;
        case ID_REMOTE_NEW_INCOMING_CONNECTION:  // Server telling the clients
                                                 // of another client
                                                 // connecting.  You can
                                                 // manually broadcast this in a
                                                 // peer to peer enviroment if
                                                 // you want.
