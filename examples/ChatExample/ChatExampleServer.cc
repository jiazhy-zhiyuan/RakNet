...
//	server->AttachPlugin(&packetLogger);
  ...
  for (unsigned int i = 0; i < server->GetNumberOfAddresses(); i++) {
    RakNet::SystemAddress sa =
        server->GetInternalID(RakNet::UNASSIGNED_SYSTEM_ADDRESS, i);
    printf("%i. %s (LAN=%i)\n", i + 1, sa.ToString(false), sa.IsLANAddress());
  }

  printf("\nMy GUID is %s\n",
         server->GetGuidFromSystemAddress(RakNet::UNASSIGNED_SYSTEM_ADDRESS)
             .ToString());
  puts(
  ...
  while (1) {
    ...
    if (kbhit()) {
      ...
      if (strcmp(message, "getconnectionlist") == 0) {
        ...
        server->GetConnectionList((RakNet::SystemAddress *)&systems,
                                  &numConnections);
    ...
    for (p = server->Receive(); p;
      ...
      switch (packetIdentifier) {
        ...
        case ID_NEW_INCOMING_CONNECTION:
          ...
          for (int index = 0; index < MAXIMUM_NUMBER_OF_INTERNAL_IDS; index++) {
            RakNet::SystemAddress internalId =
                server->GetInternalID(p->systemAddress, index);
        ...
        default:
          // The server knows the static data of all clients, so we can prefix
          // the message With the name data
          printf("%s\n", p->data);

          // Relay the message.  We prefix the name for other clients.  This
          // demonstrates That messages can be changed on the server before
          // being broadcast Sending is the same as before
          sprintf(message, "%s", p->data);
          server->Send(message, (const int)strlen(message) + 1, HIGH_PRIORITY,
                       RELIABLE_ORDERED, 0, p->systemAddress, true);
