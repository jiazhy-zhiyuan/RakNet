...
int main(void) {
  ...
  if (client) {
    ...
    client->SetSplitMessageProgressInterval(
        10000);  // Get ID_DOWNLOAD_PROGRESS notifications
  ...
  while (!quit) {
    if (server) {
      for (packet = server->Receive(); packet;
        ...
        if (packet->data[0] == ID_NEW_INCOMING_CONNECTION ||
          ...
          server->Send(text, BIG_PACKET_SIZE, LOW_PRIORITY,
                       RELIABLE_ORDERED_WITH_ACK_RECEIPT, 0,
      ...
      if (kbhit()) {
        ...
        if (ch == ' ') {
          ...
          server->Send(t, 1, MEDIUM_PRIORITY, RELIABLE_ORDERED, 1,
    ...
    if (client) {
      ...
      while (packet) {
        if (packet->data[0] == ID_DOWNLOAD_PROGRESS) {
          ...
          progressBS.ReadBits((unsigned char *)&progress,
                              BYTES_TO_BITS(sizeof(progress)), true);
          progressBS.ReadBits((unsigned char *)&total,
                              BYTES_TO_BITS(sizeof(total)), true);
          progressBS.ReadBits((unsigned char *)&partLength,
                              BYTES_TO_BITS(sizeof(partLength)), true);

          printf("Progress: msgID=%i Progress %i/%i Partsize=%i\n",
                 (unsigned char)packet->data[0], progress, total, partLength);
          ...
          if (quit == false) {
            ...
            if (repeat) {
              ...
              client->Send((const char *)&ch, 1, MEDIUM_PRIORITY,
                           RELIABLE_ORDERED, 1,
                           RakNet::UNASSIGNED_SYSTEM_ADDRESS, true);
            } else {
              quit = true;
    ...
    if (RakNet::GetTimeMS() > nextStatTime) {
      ...
      if (server) {
        ...
        if (numSystems > 0) {
          for (i = 0; i < numSystems; i++) {
            server->GetStatistics(server->GetSystemAddressFromIndex(i),
                                  &rssSender);
      ...
      if (client && server == 0 &&
          client->GetGUIDFromIndex(0) != UNASSIGNED_RAKNET_GUID) {
        client->GetStatistics(client->GetSystemAddressFromIndex(0),
                              &rssReceiver);
  ...
  if (server) {
    RakNetStatistics *rssSender2 =
        server->GetStatistics(server->GetSystemAddressFromIndex(0));
