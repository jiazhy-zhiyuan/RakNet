...
class RAK_DLL_EXPORT TCPInterface {
 public:
  ...
  /// maxIncomingConnections \param[in] threadPriority Passed to the thread
  /// creation routine. Use THREAD_PRIORITY_NORMAL for Windows. For Linux based
  /// systems, you MUST pass something reasonable based on the thread priorities
  /// for your application. \param[in] socketFamily IP version: For IPV4, use
  ...
  /// Sends a byte stream
  virtual void Send(const char *data, unsigned int length,
                    const SystemAddress &systemAddress, bool broadcast);

  // Sends a concatenated list of byte streams
  virtual bool SendList(const char **data, const unsigned int *lengths,
 ...
 protected:
  ...
  bool CreateListenSocket_WinStore8(unsigned short port,
                                    unsigned short maxIncomingConnections,
                                    unsigned short socketFamily,
                                    const char *hostAddress);
#else
  bool CreateListenSocket(unsigned short port,
                          unsigned short maxIncomingConnections,
