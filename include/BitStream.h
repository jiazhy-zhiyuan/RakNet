...
/// bits. \details BitStream is used extensively throughout RakNet and is
/// designed to be used by users as well.
///
...
/// by users as well. \sa BitStreamSample.txt
class RAK_DLL_EXPORT BitStream {
 public:
  ...
  /// allocate. \details There is no benefit to calling this, unless you know
  /// exactly how many bytes you need and it is greater than
  /// BITSTREAM_STACK_ALLOCATION_SIZE. In that case all it does is save you one
  /// or more realloc calls. \param[in] initialBytesToAllocate the number of
  /// bytes to pre-allocate.
  ...
  /// to a predefined pointer. \details Set \a _copyData to true if you want to
  /// make an internal copy of the data you are passing. Set it to false to just
  /// save a pointer to the data. You shouldn't call Write functions with \a
  /// _copyData as false, as this will write to unallocated memory 99% of the
  /// time you will use this function to cast Packet::data to a bitstream for
  /// reading, in which case you should write something as follows: \code
  ...
  /// bitstream. \details Undefine __BITSTREAM_NATIVE_END if you need endian
  /// swapping. \param[in] writeToBitstream true to write from your data to this
  ...
  /// \brief Bidirectional serialize/deserialize any integral type to/from a
  /// bitstream. \details If the current value is different from the last value
  /// the current value will be written.  Otherwise, a single bit will be
  /// written \param[in] writeToBitstream true to write from your data to this
  /// bitstream.  False to read from this bitstream and write to your data
  /// \param[in] inOutCurrentValue The current value to write
  /// \param[in] lastValue The last value to compare against.  Only used if \a
  /// writeToBitstream is true. \return true if \a writeToBitstream is true.
  /// true if \a writeToBitstream is false and the read was successful.  false
  /// if \a writeToBitstream is false and the read was not successful.
  template <class templateType>
  ...
  /// the last value is, or there is no last value. \param[in] writeToBitstream
  /// true to write from your data to this bitstream.  False to read from this
  /// bitstream and write to your data \param[in] inOutCurrentValue The current
  /// value to write \return true if \a writeToBitstream is true.  true if \a
  /// writeToBitstream is false and the read was successful.  false if \a
  ...
  /// \brief Bidirectional serialize/deserialize any integral type to/from a
  /// bitstream. \details Undefine __BITSTREAM_NATIVE_END if you need endian
  /// swapping. If you are not using __BITSTREAM_NATIVE_END the opposite is true
  /// for types larger than 1 byte For floating point, this is lossy, using 2
  /// bytes for a float and 4 for a double.  The range must be between -1 and
  /// +1. For non-floating point, this is lossless, but only has benefit if you
  /// use less than half the bits of the type \param[in] writeToBitstream true
  /// to write from your data to this bitstream.  False to read from this
  /// bitstream and write to your data \param[in] inOutTemplateVar The value to
  /// write \return true if \a writeToBitstream is true.  true if \a
  /// writeToBitstream is false and the read was successful.  false if \a
  /// writeToBitstream is false and the read was not successful.
  template <class templateType>
  ...
  /// opposite is true for types larger than 1 byte \param[in] writeToBitstream
  /// true to write from your data to this bitstream.  False to read from this
  /// bitstream and write to your data \param[in] inOutCurrentValue The current
  /// value to write \param[in] lastValue The last value to compare against.
  /// Only used if \a writeToBitstream is true. \return true if \a
  /// writeToBitstream is true.  true if \a writeToBitstream is false and the
  /// read was successful.  false if \a writeToBitstream is false and the read
  /// was not successful.
  template <class templateType>
  ...
  /// templateType &lastValue) when we have an unknown second parameter \return
  ...
  /// raw data.  This does NOT do endian swapping. \param[in] writeToBitstream
  /// true to write from your data to this bitstream.  False to read from this
  /// bitstream and write to your data \param[in] inOutByteArray a byte buffer
  ...
  /// floatMin and \a floatMax \param[in] writeToBitstream true to write from
  /// your data to this bitstream.  False to read from this bitstream and write
  /// to your data \param[in] inOutFloat The float to write \param[in] floatMin
  /// Predetermined minimum value of f \param[in] floatMax Predetermined maximum
  /// value of f
  ...
  /// to write what would otherwise be an integer (4 or 8 bytes) \param[in]
  /// writeToBitstream true to write from your data to this bitstream.  False to
  /// read from this bitstream and write to your data \param[in] value The value
  /// to serialize
  ...
  /// life of the program \param[in] writeToBitstream true to write from your
  /// data to this bitstream.  False to read from this bitstream and write to
  /// your data \param[in] value Integer value to write, which should be between
  /// \a minimum and \a maximum \param[in] minimum Minimum value of \a value
  ...
  /// (at most) 4 bytes + 3 bits instead of 12-24 bytes. \details Will further
  /// compress y or z axis aligned vectors. Accurate to 1/32767.5. \param[in]
  /// writeToBitstream true to write from your data to this bitstream.  False to
  /// read from this bitstream and write to your data \param[in] x x \param[in]
  /// y y \param[in] z z \return true if \a writeToBitstream is true.  true if
  /// \a writeToBitstream is false and the read was successful.  false if \a
  /// writeToBitstream is false and the read was not successful.
  ...
  /// instead of 12. \details Loses accuracy to about 3/10ths and only saves 2
  /// bytes, so only use if accuracy is not important. \param[in]
  /// writeToBitstream true to write from your data to this bitstream.  False to
  /// read from this bitstream and write to your data \param[in] x x \param[in]
  /// y y \param[in] z z \return true if \a writeToBitstream is true.  true if
  /// \a writeToBitstream is false and the read was successful.  false if \a
  ...
  /// bytes + 4 bits instead of 16 bytes. Slightly lossy. \param[in]
  /// writeToBitstream true to write from your data to this bitstream.  False to
  /// read from this bitstream and write to your data \param[in] w w \param[in]
  /// x x \param[in] y y \param[in] z z \return true if \a writeToBitstream is
  /// true.  true if \a writeToBitstream is false and the read was successful.
  ...
  /// bytes each. \details Use 6 bytes instead of 36 Lossy, although the result
  /// is renormalized \return true on success, false on failure.
  template <class templateType>  // templateType for this function must be a
                                 // float or double
  ...
  /// the input. \details Right aligned data means in the case of a partial
  /// byte, the bits are aligned from the right (bit 0) rather than the left (as
  /// in the normal internal representation) You would set this to true when
  ...
  /// the current value will be written.  Otherwise, a single bit will be
  /// written \param[in] currentValue The current value to write \param[in]
  ...
  /// no last value. \param[in] currentValue The current value to write
  template <class templateType>
  void WriteDelta(const templateType &currentValue);
  ...
  /// than half the bits of the type \param[in] inTemplateVar The value to write
  template <class templateType>
  ...
  /// point, this is lossless, but only has benefit if you use less than half
  /// the bits of the type If you are not using __BITSTREAM_NATIVE_END the
  /// opposite is true for types larger than 1 byte \param[in] currentValue The
  /// current value to write \param[in] lastValue The last value to compare
  /// against
  template <class templateType>
  ...
  /// for types larger than 1 byte \param[in] outTemplateVar The value to read
  /// \return true on success, false on failure.
  template <class templateType>
  ...
  /// endian swapping. \param[in] inputByteArray a byte buffer \param[in]
  /// numberOfBytes the size of \a input in bytes
  ...
  /// and \a floatMax \param[in] x The float to write \param[in] floatMin
  /// Predetermined minimum value of f \param[in] floatMax Predetermined maximum
  /// value of f
  ...
  /// what would otherwise be an integer (4 or 8 bytes) \param[in] value The
  /// value to write
  template <class serializationType, class sourceType>
  ...
  /// (maximum-minimum) is only calculated once. This does require that \a
  /// minimum and \maximum are fixed values for a given line of code for the
  /// life of the program \param[in] value Integer value to write, which should
  /// be between \a minimum and \a maximum \param[in] minimum Minimum value of
  /// \a value \param[in] maximum Maximum value of \a value \param[in]
  /// allowOutsideRange If true, all sends will take an extra bit, however value
  /// can deviate from outside \a minimum and \a maximum. If false, will assert
  /// if the value deviates. This should match the corresponding value passed to
  /// Read().
  template <class templateType>
  ...
  /// instead of 12-24 bytes. \details Will further compress y or z axis aligned
  /// vectors. Accurate to 1/32767.5. \param[in] x x \param[in] y y \param[in] z
  /// z
  template <class templateType>  // templateType for this function must be a
                                 // float or double
  ...
  template <class templateType>  // templateType for this function must be a
                                 // float or double
                                 void WriteVector(templateType x,
                                                  templateType y,
                                                  templateType z);

  /// \brief Write a normalized quaternion in 6 bytes + 4 bits instead of 16
  /// bytes.  Slightly lossy. \param[in] w w \param[in] x x \param[in] y y
  /// \param[in] z z
  template <class templateType>  // templateType for this function must be a
                                 // float or double
                                 void WriteNormQuat(templateType w,
                                                    templateType x,
                                                    templateType y,
                                                    templateType z);

  /// \brief Write an orthogonal matrix by creating a quaternion, and writing 3
  /// components of the quaternion in 2 bytes each. \details Use 6 bytes instead
  /// of 36 Lossy, although the result is renormalized
  template <class templateType>  // templateType for this function must be a
                                 // float or double
  ...
  /// with this function \param[in] output The result byte array. It should be
  /// larger than @em numberOfBytes. \param[in] numberOfBytes The number of byte
  /// to read \return true on success false if there is some missing bytes.
  ...
  /// and \a floatMax \param[in] outFloat The float to read \param[in] floatMin
  /// Predetermined minimum value of f \param[in] floatMax Predetermined maximum
  /// value of f
  ...
  /// stream, and put the value in an integer \param[in] value The value to
  /// write
  template <class serializationType, class sourceType>
  ...
  /// (maximum-minimum) is only calculated once. This does require that \a
  /// minimum and \maximum are fixed values for a given line of code for the
  /// life of the program \param[in] value Integer value to read, which should
  /// be between \a minimum and \a maximum \param[in] minimum Minimum value of
  /// \a value \param[in] maximum Maximum value of \a value \param[in]
  /// allowOutsideRange If true, all sends will take an extra bit, however value
  /// can deviate from outside \a minimum and \a maximum. If false, will assert
  ...
  /// instead of 12-24 bytes. \details Will further compress y or z axis aligned
  /// vectors. Accurate to 1/32767.5. \param[in] x x \param[in] y y \param[in] z
  /// z \return true on success, false on failure.
  template <class templateType>  // templateType for this function must be a
                                 // float or double
  ...
  /// doubles comprise a vector. \details Loses accuracy to about 3/10ths and
  /// only saves 2 bytes, so only use if accuracy is not important. \param[in] x
  /// x \param[in] y y \param[in] z z \return true on success, false on failure.
  template <class templateType>  // templateType for this function must be a
                                 // float or double
  ...
  /// bytes. \param[in] w w \param[in] x x \param[in] y y \param[in] z z \return
  /// true on success, false on failure.
  template <class templateType>  // templateType for this function must be a
                                 // float or double
                                 bool ReadNormQuat(templateType &w,
                                                   templateType &x,
                                                   templateType &y,
                                                   templateType &z);

  /// \brief Read an orthogonal matrix from a quaternion, reading 3 components
  /// of the quaternion in 2 bytes each and extrapolatig the 4th. \details Use 6
  /// bytes instead of 36 Lossy, although the result is renormalized \return
  /// true on success, false on failure.
  template <class templateType>  // templateType for this function must be a
                                 // float or double
                                 bool ReadOrthMatrix(
  ...
  /// write beyond maxBytesToWrite \param[in] inByteArray The data \param[in]
  /// inputLength The size of input. \param[in] maxBytesToWrite Max bytes to
  /// write
  ...
  /// numberOfBytesToRead \param[in] numberOfBytesToRead The number of byte to
  /// read from the internal state \return true if there is enough byte.
  ...
  /// using new, rather than assuming it is safe to write to \param[in]
  /// outByteArray outByteArray will be deleted if it is not a pointer to 0
  /// \return true on success, false on failure.
...
inline void BitStream::Write(const uint24_t &inTemplateVar) {
  ...
  if (IsBigEndian() == false) {
    data[(numberOfBitsUsed >> 3) + 0] =
        ((unsigned char *)&inTemplateVar.val)[0];
    data[(numberOfBitsUsed >> 3) + 1] =
        ((unsigned char *)&inTemplateVar.val)[1];
    data[(numberOfBitsUsed >> 3) + 2] =
        ((unsigned char *)&inTemplateVar.val)[2];
  } else {
    data[(numberOfBitsUsed >> 3) + 0] =
        ((unsigned char *)&inTemplateVar.val)[3];
    data[(numberOfBitsUsed >> 3) + 1] =
        ((unsigned char *)&inTemplateVar.val)[2];
    data[(numberOfBitsUsed >> 3) + 2] =
        ((unsigned char *)&inTemplateVar.val)[1];
...
/// last value. \param[in] currentValue The current value to write
template <class templateType>
inline void BitStream::WriteDelta(const templateType &currentValue) {
  Write(true);
  Write(currentValue);
...
/// types larger than 1 byte \param[in] inTemplateVar The value to write
template <class templateType>
...
/// is lossless, but only has benefit if you use less than half the bits of the
/// type If you are not using __BITSTREAM_NATIVE_END the opposite is true for
/// types larger than 1 byte \param[in] currentValue The current value to write
/// \param[in] lastValue The last value to compare against
template <class templateType>
...
/// __BITSTREAM_NATIVE_END if you need endian swapping. \param[in]
/// outTemplateVar The value to read
template <class templateType>
...
/// is lossless, but only has benefit if you use less than half the bits of the
/// type If you are not using __BITSTREAM_NATIVE_END the opposite is true for
/// types larger than 1 byte \param[in] outTemplateVar The value to read
template <class templateType>
...
template <class templateType>  // templateType for this function must be a float
  ...
  m00 =
      (templateType)(sqx - sqy - sqz + sqw);  // since sqw + sqx + sqy + sqz =1
