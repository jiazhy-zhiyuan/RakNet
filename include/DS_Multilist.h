...
/// holds lists. \param[in] _MultilistType What type of list this is, \sa
/// MultilistType \param[in] _DataType What type of data this list holds.
...
/// defined \param[in] _IndexType What variable type to use for indices
...
class RAK_DLL_EXPORT Multilist {
 public:
  ...
  /// element \details Index[0] returns the same as Pop() for a queue. Same as
  /// PopOpposite() for the list and ordered list
  ...
  /// nothing as the list is assumed to already be sorted. \details However, if
  /// \a force is true, it will also resort the ordered list, useful if the
  /// comparison operator between _KeyType and _DataType would now return
  /// different results Once the list is sorted, further operations to lookup by
  /// key will be log2(n) until the list is modified
  ...
  /// state. \details Doesn't actually check for sortedness, just if Sort() was
  /// recently called, or MultilistType is ML_ORDERED_LIST
  ...
  /// do anything \param[in] mlType Any value of the enum MultilistType, except
  /// ML_VARIABLE_DURING_RUNTIME
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType,
          class _IndexType>
_DataType &Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::
operator[](const _IndexType position) const {
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType,
          class _IndexType>
_DataType &Multilist<_MultilistType, _DataType, _KeyType,
                     _IndexType>::PopOpposite(const char *file,
                                              unsigned int line) {
  RakAssert(IsEmpty() == false);
  if (GetMultilistType() == ML_UNORDERED_LIST ||
      GetMultilistType() == ML_STACK || GetMultilistType() == ML_ORDERED_LIST) {
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType,
          class _IndexType>
_DataType &Multilist<_MultilistType, _DataType, _KeyType,
                     _IndexType>::PeekOpposite(void) const {
  RakAssert(IsEmpty() == false);
  if (GetMultilistType() == ML_UNORDERED_LIST ||
      GetMultilistType() == ML_STACK || GetMultilistType() == ML_ORDERED_LIST) {
    return data[0];
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType,
          class _IndexType>
_IndexType Multilist<_MultilistType, _DataType, _KeyType,
                     _IndexType>::GetIndexOf(_KeyType key) const {
  _IndexType i;
  if (IsSorted()) {
    bool objectExists;
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType,
          class _IndexType>
MultilistType Multilist<_MultilistType, _DataType, _KeyType,
                        _IndexType>::GetMultilistType(void) const {
  if (_MultilistType == ML_VARIABLE_DURING_RUNTIME)
