...
#pragma warning( \
    disable : 4127)  // warning C4127: conditional expression is constant
#endif
  if (allow_unlinkedNodes == false)  // If we do not allow _unlinked nodes, then
                                     // if there are no connections, remove the
                                     // node
  {
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
unsigned WeightedGraph<node_type, weight_type,
                       allow_unlinkedNodes>::GetNodeCount(void) const {
