...
class RAK_DLL_EXPORT StatisticsHistory {
 public:
  ...
  bool AddValueByObjectID(uint64_t objectId, RakString key, SHValueType val,
                          Time curTime, bool combineEqualTimes);
  void AddValueByIndex(unsigned int index, RakString key, SHValueType val,
                       Time curTime, bool combineEqualTimes);
  SHErrorCode GetHistoryForKey(uint64_t objectId, RakString key,
                               TimeAndValueQueue **values, Time curTime) const;
  ...
  struct TimeAndValueQueue {
    ...
    static void MergeSets(const TimeAndValueQueue *lhs,
...
/// \brief Input numerical values over time. Get sum, average, highest, lowest,
/// standard deviation on recent or all-time values \ingroup PLUGINS_GROUP
class RAK_DLL_EXPORT StatisticsHistoryPlugin : public PluginInterface2 {
 public:
  ...
  void SetTrackConnections(bool _addNewConnections,
