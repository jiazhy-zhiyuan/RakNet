...
class RAK_DLL_EXPORT BPlusTree {
 ...
 protected:
  ...
  Page<KeyType, DataType, order> *InsertIntoNode(
      const KeyType key, const DataType &childData, int insertionIndex,
...
#pragma warning( \
    ...
    if (order <= 3 && branchIndex > 0 &&
        cur->children[branchIndex]->isLeaf)  // With order==2 it is possible to
                                             // delete data[0], which is not
...
void main(void)
{
        ...
        for (testSize=0; testSize < 514; testSize++)
        {
                ...
                for (i=0; i<testSize; i++)
                {
                        btree.Delete(haveList[0]); // Asserts on 8th call.
Fails on going to remove 8 (7th call) removedList.Insert(haveList[0]);
