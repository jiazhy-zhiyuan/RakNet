...
class RAK_DLL_EXPORT SingleProducerConsumer {
 public:
  ...
  /// functions must be called in the same thread. \return A pointer to a block
  /// of data you can write to.
  ...
  /// must be called in the same thread. \retval 0 No data is availble to read
  ...
  /// This function will estimate how many elements are waiting to be read.
  /// It's threadsafe enough that the value returned is stable, but not
  /// threadsafe enough to give accurate results. \return An ESTIMATE of how
  /// many data elements are waiting to be read
  int Size(void) const;
