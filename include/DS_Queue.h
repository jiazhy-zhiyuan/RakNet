...
class RAK_DLL_EXPORT Queue {
 public:
  ...
  void PushAtHead(const queue_type& input, unsigned index, const char* file,
                  unsigned int line);
  queue_type& operator[](unsigned int position) const;  // Not a normal thing
                                                        // you do with a queue
                                                        // but can be used for
                                                        // efficiency
  void RemoveAtIndex(unsigned int position);  // Not a normal thing you do with
                                              // a queue but can be used for
                                              // efficiency
...
void Queue<queue_type>::Compress(const char* file, unsigned int line) {
  ...
  while (newAllocationSize <= Size())
    newAllocationSize <<= 1;  // Must be a better way to do this but I'm too
                              // dumb to figure it out quickly :)
