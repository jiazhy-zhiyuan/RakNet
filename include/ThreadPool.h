...
struct RAK_DLL_EXPORT ThreadPool {
  ...
  /// Pass 0 if not needed. \param[in] _perThreadDeinit User callback to destroy
  /// data stored per thread, created by _perThreadInit.  Pass 0 if not needed.
  ...
  /// iterate through outputQueue and deallocate it there. \param[in]
  /// workerThreadCallback The function to call from the thread \param[in]
  /// inputData The parameter to pass to \a userCallback
  ...
  /// should still check HasOutput for the real value. \return true if output is
  /// probably waiting, false otherwise
  ...
  /// still check HasInput for the real value. \return true if input is probably
  ...
  /// will assert. \return The output of \a userCallback.  If you have different
  /// output signatures, it is up to you to encode the data to indicate this
