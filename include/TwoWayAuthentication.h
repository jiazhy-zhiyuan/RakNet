...
/// MessageFilter first in the list of plugins \note If other systems challenges
/// us, and fails, you will get
...
class RAK_DLL_EXPORT TwoWayAuthentication : public PluginInterface2 {
 public:
  ...
  /// by \a identifier. \a identifier is transmitted in plaintext with the
  /// request. It is only needed because the system supports multiple password.
  /// It is used to only hash against once password on the remote system, rather
  /// than having to hash against every known password. \param[in] identifier A
  /// unique identifier representing this password. This is transmitted in
  /// plaintext and should be considered insecure \param[in] password The
  /// password to add \return True on success, false on identifier==password,
  /// either identifier or password is blank, or identifier is already in use
  ...
  /// pair as this system. \param[in] identifier A unique identifier
  /// representing this password. This is transmitted in plaintext and should be
  /// considered insecure \return True on success, false on remote system not
  /// connected, or identifier not previously added with AddPassword()
