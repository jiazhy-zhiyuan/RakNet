...
class RAK_DLL_EXPORT CommandParserInterface {
 public:
  ...
  /// string \return The name that you return.
  ...
  /// gracefully or forcefully \param[in] systemAddress The player that has
  /// disconnected. \param[in] transport The transport interface that sent us
  /// this information.
  ...
  /// your parser to \a systemAddress \param[in] transport The transport
  /// interface we can use to write to \param[in] systemAddress The player that
  /// requested help.
  ...
  /// processing you wish. \param[in] command The command to process \param[in]
  /// numParameters How many parameters were passed along with the command
  /// \param[in] parameterList The list of parameters.  parameterList[0] is the
  /// first parameter and so on. \param[in] transport The transport interface we
  /// can use to write to \param[in] systemAddress The player that sent this
  /// command. \param[in] originalString The string that was actually sent over
  /// the network, in case you want to do your own parsing
  ...
  /// toggled on and off \param[out] numParameters How many pointers were
  /// written to \a parameterList \param[out] parameterList An array of pointers
  /// to characters.  Will hold pointers to locations inside \a str \param[in]
  /// parameterListLength How big the \a parameterList array is
  ...
  /// each struct \param[in] transport The transport interface we can use to
  /// write to \param[in] systemAddress The player to write to
  ...
  /// CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS \param[in] command A
  /// pointer to a STATIC string that has your command.  I keep a copy of the
  /// pointer here so don't deallocate the string. \param[in] commandHelp A
  /// pointer to a STATIC string that has the help information for your command.
  /// I keep a copy of the pointer here so don't deallocate the string.
  ...
  /// res ) of your operation \details This is not necessary to call, but makes
  /// it easier to return results of function calls. \param[in] res The result
  /// to write \param[in] command The command that this result came from
  ...
  /// function that has no return value. \details This is not necessary to call,
  /// but makes it easier to return results of function calls. \param[in] res
  /// The result to write \param[in] command The command that this result came
  /// from \param[in] transport The transport interface that will be written to
  /// \param[in] systemAddress The player this result will be sent to
