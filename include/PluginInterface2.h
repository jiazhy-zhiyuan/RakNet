...
/// with the message \ingroup PLUGIN_INTERFACE_GROUP
...
class RAK_DLL_EXPORT PluginInterface2 {
 public:
  ...
  /// RakPeer::CloseConnection() for a particular system \param[in]
  /// systemAddress The system whose connection was closed \param[in] rakNetGuid
  /// The guid of the specified system \param[in] lostConnectionReason How the
  /// connection was closed: manually, connection lost, or notification of
  /// disconnection
  ...
  /// reliability layer \pre To be called, UsesReliabilityLayer() must return
  /// true \param[in] data The data being sent \param[in] bitsUsed How many bits
  /// long \a data is \param[in] remoteSystemAddress Which system this message
  /// is being sent to
  ...
  /// through the reliability layer \pre To be called, UsesReliabilityLayer()
  /// must return true \param[in] data The data being sent \param[in] bitsUsed
  /// How many bits long \a data is \param[in] remoteSystemAddress Which system
  ...
  virtual void OnReliabilityLayerNotification(const char *errorMessage,
  ...
  /// message. \param[in] remoteSystemAddress The player we sent or got this
  /// packet from \param[in] time The current time as returned by
  /// RakNet::GetTimeMS() \param[in] isSend Is this callback representing a send
  /// event or receive event?
  ...
  /// is \param[in] remoteSystemAddress The player we sent or got this packet
  /// from \param[in] time The current time as returned by RakNet::GetTimeMS()
