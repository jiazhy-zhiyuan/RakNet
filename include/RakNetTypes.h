...
struct RAK_DLL_EXPORT SystemAddress {
  ...
  /// portDelineator, for example "192.0.2.1|1234" \param[in] str A printable IP
  /// string, for example "192.0.2.1" or "2001:db8:63b3:1::3490". Pass 0 for \a
  /// str to set to UNASSIGNED_SYSTEM_ADDRESS \param[in] portDelineator if \a
  /// str contains a port, delineate the port with this character.
  /// portDelineator should not be '.', ':', '%', '-', '/', a number, or a-f
  ...
  /// 6 here to do the conversion \note The current port is unchanged if a port
  /// is not specified in \a str \return True on success, false on ipVersion
