...
/// operation, that operation is silently rejected \ingroup CLOUD_GROUP
class RAK_DLL_EXPORT CloudServerQueryFilter {
 public:
  ...
  /// uploaded \return true to allow, false to reject
  ...
  virtual bool OnGetRequest(
      RakNetGUID clientGuid, SystemAddress clientAddress, CloudQuery &query,
  ...
  virtual bool OnUnsubscribeRequest(
      RakNetGUID clientGuid, SystemAddress clientAddress,
      DataStructures::List<CloudKey> &cloudKeys,
      DataStructures::List<RakNetGUID> &specificSystems) = 0;
};

/// \brief Stores client data, and allows cross-server communication to retrieve
/// this data \ingroup CLOUD_GROUP
class RAK_DLL_EXPORT CloudServer : public PluginInterface2, CloudAllocator {
 public:
  ...
  /// result in no rows downloaded, then one row will be returned. \param[in]
  /// bytes Max bytes a client can download from a single Get(). 0 means
  /// unlimited.
  ...
  /// sufficient for this to work. \note This sends subscription data to the
  /// other system, using RELIABLE_ORDERED on channel 0 \param[in]
  /// systemIdentifier Identifier of the remote system
  ...
  /// queries \param[in] forcedAddress The systmeAddress to return in queries.
  /// Use UNASSIGNED_SYSTEM_ADDRESS (default) to use what RakPeer returns
  ...
  /// added, the function silently fails \param[in] filter An externally
  /// allocated instance of CloudServerQueryFilter. The instance must remain
  /// valid until it is removed with RemoveQueryFilter() or
  ...
  /// delete the instance, if necessary \param[in] filter An externally
  /// allocated instance of CloudServerQueryFilter. The instance must remain
  /// valid until it is removed with RemoveQueryFilter() or
  /// RemoveAllQueryFilters()
 ...
 protected:
  ...
  struct CloudData {
    ...
    unsigned char *allocatedData;  // Uses allocatedData instead of stackData if
