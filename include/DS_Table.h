...
class RAK_DLL_EXPORT Table {
 public:
  ...
  /// as the sort key \param[in] rowId The UNIQUE primary key for the row.  This
  /// can never be changed. \param[in] initialCellValues Initial values to give
  /// the row (optional) \return The newly added row
  ...
  /// directly. \note Row pointers do not change, so you can also write directly
  /// to the rows for more efficiency.
  /// \param[in] rowId The ID of the row
  ...
  /// function. \param[in] rowId The ID of the row \return The desired row, or 0
  /// if no such row.
  ...
  /// and rows. \param[in] columnSubset An array of column indices.  Only
  /// columns in this array are returned.  Pass 0 for all columns \param[in]
  /// numColumnSubset The number of elements in \a columnSubset \param[in]
  /// inclusionFilters An array of FilterQuery.  All filters must pass for the
  /// row to be returned. \param[in] numInclusionFilters The number of elements
  /// in \a inclusionFilters \param[in] rowIds An arrow of row IDs.  Only these
  /// rows with these IDs are returned.  Pass 0 for all rows. \param[in]
  /// numRowIDs The number of elements in \a rowIds \param[out] result The
  /// result of the query.  If no rows are returned, the table will only have
  /// columns.
  ...
  /// to perform on the table \param[in] numColumnSubset The number of elements
  /// in \a numSortQueries \param[out] out The address of an array of Rows,
  /// which will receive the sorted output.  The array must be long enough to
  /// contain all returned rows, up to GetRowCount()
  ...
  /// still print the delineator. \param[in] inputRow The row to print
