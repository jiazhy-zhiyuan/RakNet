...
class ReliabilityLayer  //<ReliabilityLayer>
{
 public:
  ...
  /// or 10 seconds in release and 30,000 or 30 seconds in debug. \param[in]
  /// time Time, in MS
  void SetTimeoutTime(RakNet::TimeMS time);

  /// Returns the value passed to SetTimeoutTime. or the default if it was never
  /// called \param[out] the value passed to SetTimeoutTime
  ...
  /// relational ordering and sequencing of packets. \param[in] makeDataCopy If
  /// true \a data will be copied.  Otherwise, only a pointer will be stored.
  /// \param[in] MTUSize maximum datagram size
  ...
  /// reliability types that contain RECEIPT in the name \return True or false
  ...
  /// send. \param[in] s the communication  end point \param[in] systemAddress
  /// The Unique Player Identifier who shouldhave sent some packets \param[in]
  /// MTUSize maximum datagram size \param[in] time current system time
  /// \param[in] maxBitsPerSecond if non-zero, enforces that outgoing bandwidth
  /// does not exceed this amount \param[in] messageHandlerList A list of
  /// registered plugins
  void Update(RakNetSocket2 *s, SystemAddress &systemAddress, int MTUSize,
 ...
 private:
  ...
  void SendBitStream(RakNetSocket2 *s, SystemAddress &systemAddress,
