...
class RAK_DLL_EXPORT UDPForwarder {
 public:
  ...
  /// bandwidth per forwarded address. \param[in] maxEntries The maximum number
  /// of simultaneous forwarding entries. Defaults to 64 (32 connections)
  ...
  /// timeoutOnNoDataMS, plus an some extra seconds for latency \param[in]
  /// source The source IP and port \param[in] destination Where to forward to
  /// (and vice-versa) \param[in] timeoutOnNoDataMS If no messages are forwarded
  /// for this many MS, then automatically remove this entry. \param[in]
  /// forceHostAddress Force binding on a particular address. 0 to use any.
  /// \param[in] socketFamily IP version: For IPV4, use AF_INET (default). For
  /// IPV6, use AF_INET6. To autoselect, use AF_UNSPEC. \param[out]
  /// forwardingPort New opened port for forwarding \param[out] forwardingSocket
  /// New opened socket for forwarding \return UDPForwarderResult
