...
class RAK_DLL_EXPORT HTTPConnection2 : public PluginInterface2 {
 public:
  ...
  /// TCPInterface::Start() called \param[in] stringToTransmit What string to
  /// transmit. See RakString::FormatForPOST(), RakString::FormatForGET(),
  /// RakString::FormatForDELETE() \param[in] host The IP address to connect to
  /// \param[in] port The port to connect to
  /// \param[in] useSSL If to use SSL to connect. OPEN_SSL_CLIENT_SUPPORT must
  /// be defined to 1 in RakNetDefines.h or RakNetDefinesOverrides.h \param[in]
  /// ipVersion 4 for IPV4, 6 for IPV6 \param[in] useAddress Assume we are
  /// connected to this address and send to it, rather than do a lookup
  /// \param[in] userData
  ...
  bool TransmitRequest(const char *stringToTransmit, const char *host,
                       unsigned short port = 80, bool useSSL = false,
                       int ipVersion = 4,
  ...
  /// OnLostConnection() \param[out] stringTransmitted The original string
  /// transmitted \param[out] hostTransmitted The parameter of the same name
  /// passed to TransmitRequest() \param[out] responseReceived The response, if
  /// any \param[out] hostReceived The SystemAddress from ProcessTCPPacket() or
  /// OnLostConnection() \param[out] contentOffset The offset from the start of
  /// responseReceived to the data body. Equivalent to searching for \r\n\r\n in
  /// responseReceived. \param[out] userData Whatever you passed to
  /// TransmitRequest \return true if there was a response. false if not.
