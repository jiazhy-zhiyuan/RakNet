...
/// call cannot execute \details Error code follows packet ID
/// ID_RPC_REMOTE_ERROR, that is packet->data[1]<BR> Name of the function will
/// be appended starting at packet->data[2] \ingroup RPC_PLUGIN_GROUP
enum RPCErrorCodes {
...
/// and a string. \details It is for users that want to use RPC, but do not want
/// to use boost. You do not have the automatic serialization or other features
/// of RPC3, and C++ member calls are not supported. \note You cannot use RPC4
/// at the same time as RPC3Plugin \ingroup RPC_PLUGIN_GROUP
class RAK_DLL_EXPORT RPC4 : public PluginInterface2 {
 public:
  ...
  /// RakNet got. \sa RegisterPacketCallback() \param[in] uniqueID Identifier to
  /// be associated with \a functionPointer. If this identifier is already in
  /// use, the call will return false. \param[in] functionPointer C function
  /// pointer to be called \return True if the hash of uniqueID is not in use,
  /// false otherwise.
  ...
  /// occurs, all slots with the same identifier are called. \param[in]
  /// sharedIdentifier A string to identify the slot. Recommended to be the same
  /// as the name of the function. \param[in] functionPtr Pointer to the
  /// function. For C, just pass the name of the function. For C++, use
  /// ARPC_REGISTER_CPP_FUNCTION \param[in] callPriority Slots are called by
  /// order of the highest callPriority first. For slots with the same priority,
  ...
  /// RegisterFunction() \param[in] Identifier originally passed to
  /// RegisterFunction() \return True if the hash of uniqueID was in use, and
  /// hence removed. false otherwise.
  ...
  /// RegisterLocalCallback() \param[in] messageId Identifier passed as
  /// messageId to RegisterLocalCallback() \return True if the combination of
  /// uniqueID and messageId was in use, and hence removed
  ...
  /// RakPeerInterface::SendLoopback() \param[in] Identifier originally passed
  /// to RegisterFunction() on the local system \param[in] bitStream bitStream
  /// encoded data to send to the function callback
  ...
  /// the remote system(s) \param[in] bitStream bitStream encoded data to send
  /// to the function callback \param[in] priority See RakPeerInterface::Send()
  ...
  /// system \note This function does not return until the remote system
  /// responds, disconnects, or was never connected to begin with \param[in]
  /// Identifier originally passed to RegisterBlockingFunction() on the remote
  /// system(s) \param[in] bitStream bitStream encoded data to send to the
  /// function callback \param[in] priority See RakPeerInterface::Send()
  /// \param[in] reliability See RakPeerInterface::Send()
  /// \param[in] orderingChannel See RakPeerInterface::Send()
  /// \param[in] systemIdentifier See RakPeerInterface::Send()
  /// \param[out] returnData Written to by the function registered with
  /// RegisterBlockingFunction. \return true if successfully called. False on
  /// disconnect, function not registered, or not connected to begin with
  ...
  /// with RegisterSlot() \param[in] sharedIdentifier parameter of the same name
  /// passed to RegisterSlot() on the remote system \param[in] bitStream
  /// bitStream encoded data to send to the function callback \param[in]
  /// priority See RakPeerInterface::Send() \param[in] reliability See
  /// RakPeerInterface::Send() \param[in] orderingChannel See
  /// RakPeerInterface::Send() \param[in] systemIdentifier See
  /// RakPeerInterface::Send() \param[in] broadcast See RakPeerInterface::Send()
