...
struct UDPProxyServerResultHandler {
  ...
  /// UDPProxyServer::LoginToCoordinator() \param[out] proxyServer The plugin
  ...
  /// UDPProxyCoordinator::SetRemoteLoginPassword() \param[out] usedPassword The
  /// password we passed to UDPProxyServer::LoginToCoordinator() \param[out]
  /// proxyServer The plugin calling this callback
...
/// coordinator will relay our forwarding request. \ingroup UDP_PROXY_GROUP
class RAK_DLL_EXPORT UDPProxyServer : public PluginInterface2 {
 public:
  ...
  /// UDPProxyCoordinator::SetRemoteLoginPassword() \returns false if already
  /// logged in, or logging in. Returns true otherwise
  ...
  /// to connect to \param[in] ip IP address to report in
