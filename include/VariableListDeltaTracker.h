...
class VariableListDeltaTracker {
 public:
  ...
  /// write) \pre Call StartWrite() before doing the first of a series of calls
  /// to WriteVar or other functions that call WriteVar \note Variables must be
  /// of the same type, written in the same order, each time
  ...
  template <class VarType>
  static bool ReadVarFromBitstream(VarType &varData,
                                   RakNet::BitStream *bitStream) {
    bool wasWritten;
