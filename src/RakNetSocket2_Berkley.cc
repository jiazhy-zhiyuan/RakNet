...
void RNS2_Berkley::SetSocketOptions(void) {
  ...
  r = setsockopt__(rns2Socket, SOL_SOCKET, SO_RCVBUF, (char *)&sock_opt,
                   sizeof(sock_opt));
  RakAssert(r == 0);
  ...
  sock_opt = 0;
  r = setsockopt__(rns2Socket, SOL_SOCKET, SO_LINGER, (char *)&sock_opt,
                   sizeof(sock_opt));
  ...
  sock_opt = 1024 * 16;
  r = setsockopt__(rns2Socket, SOL_SOCKET, SO_SNDBUF, (char *)&sock_opt,
                   sizeof(sock_opt));
  RakAssert(r == 0);
}
...
void RNS2_Berkley::SetBroadcastSocket(int broadcast) {
  setsockopt__(rns2Socket, SOL_SOCKET, SO_BROADCAST, (char *)&broadcast,
               sizeof(broadcast));
}
void RNS2_Berkley::SetIPHdrIncl(int ipHdrIncl) {
  setsockopt__(rns2Socket, IPPROTO_IP, IP_HDRINCL, (char *)&ipHdrIncl,
               sizeof(ipHdrIncl));
}
...
#endif
  setsockopt__(rns2Socket, boundAddress.GetIPPROTO(), IP_DONTFRAGMENT,
               (char *)&opt, sizeof(opt));
#endif
}

void RNS2_Berkley::GetSystemAddressIPV4(RNS2Socket rns2Socket,
...
RNS2BindResult RNS2_Berkley::BindSharedIPV4(
    RNS2_BerkleyBindParameters *bindParameters, const char *file,
    unsigned int line) {
  (void)file;
  (void)line;
  ...
  rns2Socket = (int)socket__(bindParameters->addressFamily,
                             bindParameters->type, bindParameters->protocol);
  ...
  if (ret <= -1) {
    ...
    switch (ret) {
      ...
      case EINVAL:
        RAKNET_DEBUG_PRINTF(
            "bind__(): The addrlen is wrong, or the socket was not in the "
            "AF_UNIX family.\n");
        break;
      case EROFS:
        RAKNET_DEBUG_PRINTF(
            "bind__(): The socket inode would reside on a read-only file "
            "system.\n");
        break;
      case EFAULT:
        RAKNET_DEBUG_PRINTF(
            "bind__(): my_addr points outside the user's accessible address "
            "space.\n");
        break;
      case ENAMETOOLONG:
      ...
      case EACCES:
        ...
        RAKNET_DEBUG_PRINTF(
            "bind__(): Search permission is denied on a component of the path "
            "prefix.\n");
        break;

      case ELOOP:
        RAKNET_DEBUG_PRINTF(
            "bind__(): Too many symbolic links were encountered in resolving "
            "my_addr.\n");
        break;
...
RNS2BindResult RNS2_Berkley::BindSharedIPV4And6(
  ...
  for (aip = servinfo; aip != NULL; aip = aip->ai_next) {
    ...
    if (ret >= 0) {
      // Is this valid?
      memcpy(&boundAddress.address.addr6, aip->ai_addr,
...
void RNS2_Berkley::RecvFromBlockingIPV4And6(RNS2RecvStruct *recvFromStruct) {
  ...
  recvFromStruct->bytesRead =
      recvfrom__(rns2Socket, recvFromStruct->data, dataOutSize, flag,
...

  {
    if (their_addr.ss_family == AF_INET) {
      memcpy(&recvFromStruct->systemAddress.address.addr4,
             (sockaddr_in *)&their_addr, sizeof(sockaddr_in));
      ...
      memcpy(&recvFromStruct->systemAddress.address.addr6,
             (sockaddr_in6 *)&their_addr, sizeof(sockaddr_in6));
      recvFromStruct->systemAddress.debugPort =
...
void RNS2_Berkley::RecvFromBlockingIPV4(RNS2RecvStruct *recvFromStruct) {
  ...
  recvFromStruct->bytesRead =
      recvfrom__(GetSocket(), recvFromStruct->data,
                 sizeof(recvFromStruct->data), flag, sockAddrPtr, socketlenPtr);
