...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
  if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
    ...
    if (JoinSpecificTeamCheck(teamSelection.teamParameter.specificTeamToJoin,
                              false) == false)
      return false;
...
bool TM_TeamMember::RequestTeamSwitch(TM_Team *teamToJoin,
                                      TM_Team *teamToLeave) {
  ...
  world->GetTeamManager()->SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED,
                                       0, world->GetHost(), false);
...
void TM_TeamMember::GetLastTeams(
    DataStructures::List<TM_Team *> &_teams) const {
  _teams = lastTeams;
}
...
bool TM_TeamMember::DeserializeConstruction(TeamManager *teamManager,
  ...
  for (unsigned int i = 0; i < teamsRequestedSize; i++) {
    ...
    rt.requestIndex = world->teamRequestIndex++;  // In case whenRequested is
                                                  // the same between two teams
                                                  // when sorting team requests
...
bool TM_TeamMember::SwitchSpecificTeamCheck(TM_Team *teamToJoin,
                                            TM_Team *teamToLeave,
                                            bool ignoreRequested) const {
...
void TM_TeamMember::UpdateTeamsRequestedToAny(void) {
  ...
  joinAnyRequestIndex =
      world->teamRequestIndex++;  // In case whenRequested is the same between
...
void TM_TeamMember::AddToRequestedTeams(TM_Team *teamToJoin) {
  ...
  rt.teamToLeave = 0;
  rt.whenRequested = RakNet::GetTime();
  rt.requestIndex = world->teamRequestIndex++;  // In case whenRequested is the
                                                // same between two teams when
                                                // sorting team requests
  teamsRequested.Push(rt, _FILE_AND_LINE_);
}
...
bool TM_Team::SetMemberLimit(TeamMemberLimit _teamMemberLimit,
                             NoTeamId noTeamId) {
...
bool TM_Team::DeserializeConstruction(TeamManager *teamManager,
                                      BitStream *constructionBitstream) {
  WorldId worldId;
  constructionBitstream->Read(worldId);
  TM_World *world = teamManager->GetWorldWithId(worldId);
...
void TM_World::AddParticipant(RakNetGUID rakNetGUID) {
  ...
  if (GetTeamManager()->GetMyGUIDUnified() == GetHost()) {
    ...
    teamManager->SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
                             rakNetGUID, false);
  }
}
...
void TM_World::ReferenceTeam(TM_Team *team, NetworkID networkId,
                             bool applyBalancing) {
  unsigned int i;
  for (i = 0; i < teams.Size(); i++) {
...
void TM_World::ReferenceTeamMember(TM_TeamMember *teamMember,
                                   NetworkID networkId) {
  unsigned int i;
  for (i = 0; i < teamMembers.Size(); i++) {
...
bool TM_World::SetBalanceTeams(bool balanceTeams, NoTeamId noTeamId) {
  ...
  bsOut.Write(noTeamId);
  GetTeamManager()->SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
                                GetHost(), false);
...
void TM_World::OnNewConnection(const SystemAddress &systemAddress,
  ...
  for (i = 0; i < teams.Size(); i++) {
    ...
    while (team->GetMemberLimitSetting() < team->GetTeamMembersCount() ||
      ...
      if (teamIndex == (unsigned int)-1) {
        ...
        teamManager->RemoveFromTeamsRequestedAndAddTeam(teamMember, teamToJoin,
                                                        true, team);
...
void TM_World::FillRequestedSlots(void) {
  ...
  TM_Team *team;
  TM_TeamMember *teamMember;
  DataStructures::OrderedList<TM_World::JoinRequestHelper,
  ...
  for (joinRequestIndex = 0; joinRequestIndex < joinRequests.Size();
    ...
    if (teamIndex != (unsigned int)-1) {
      ...
      if (teamMember->teamsRequested.Size() == 0) {
        ...
        int teamJoined = JoinSpecificTeam(teamMember, team, isSwitch,
                                          teamToLeave, teamsWeAreLeaving);
...
void TM_World::BroadcastToParticipants(RakNet::BitStream *bsOut,
...
void TM_World::BroadcastToParticipants(unsigned char *data, const int length,
  ...
  for (unsigned int i = 0; i < participants.Size(); i++) {
    if (participants[i] == exclusionGuid) continue;
    teamManager->SendUnified((const char *)data, length, HIGH_PRIORITY,
                             RELIABLE_ORDERED, 0, participants[i], false);
  }
}
...
int TM_World::JoinSpecificTeam(
    TM_TeamMember *teamMember, TM_Team *team, bool isTeamSwitch,
    ...
    if (balanceTeamsIsActive == false || teamsWeAreLeaving.Size() == 0) {
      ...
      if (teamsWeAreLeaving.Size() == 1) {
        ...
        if (j != (unsigned int)-1) {
          ...
          teamManager->RemoveFromTeamsRequestedAndAddTeam(teamMember, team,
                                                          true, 0);
          teamManager->RemoveFromTeamsRequestedAndAddTeam(
...
void TeamManager::EncodeTeamFull(RakNet::BitStream *bitStream,
...
void TeamManager::DecomposeTeamFull(Packet *packet, TM_World **world,
                                    TM_TeamMember **teamMember, TM_Team **team,
  ...
  DecomposeTeamFullOrLocked(&bsIn, world, teamMember, team, currentMembers,
                            memberLimitIncludingBalancing, balancingIsActive,
                            joinPermissions);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TeamManager::EncodeTeamLocked(RakNet::BitStream *bitStream,
                                   TM_TeamMember *teamMember, TM_Team *team) {
...
void TeamManager::EncodeTeamFullOrLocked(RakNet::BitStream *bitStream,
                                         TM_TeamMember *teamMember,
                                         TM_Team *team) {
...
void TeamManager::DecomposeTeamFullOrLocked(
    RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember,
...
void TeamManager::DecomposeTeamLocked(Packet *packet, TM_World **world,
                                      TM_TeamMember **teamMember,
                                      TM_Team **team, uint16_t &currentMembers,
                                      uint16_t &memberLimitIncludingBalancing,
                                      bool &balancingIsActive,
...
void TeamManager::EncodeTeamAssigned(RakNet::BitStream *bitStream,
                                     TM_TeamMember *teamMember) {
  bitStream->Write(teamMember->world->GetWorldId());
  bitStream->Write(teamMember->GetNetworkID());
...
void TeamManager::ProcessTeamAssigned(RakNet::BitStream *bsIn) {
  ...
  DecodeTeamAssigned(bsIn, &world, &teamMember, noTeamId, joinTeamType, newTeam,
                     teamsLeft, teamsJoined);
  if (teamMember) {
    teamMember->StoreLastTeams();
...
void TeamManager::DecodeTeamAssigned(Packet *packet, TM_World **world,
                                     TM_TeamMember **teamMember) {
  WorldId worldId;
  NetworkID teamMemberId;
...
void TeamManager::DecodeTeamCancelled(Packet *packet, TM_World **world,
                                      TM_TeamMember **teamMember,
...
void TeamManager::DecodeTeamAssigned(
    BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember,
...
void TeamManager::OnClosedConnection(
    ...
    PI2_LostConnectionReason lostConnectionReason) {
  for (unsigned int i = 0; i < worldsList.Size(); i++) {
    worldsList[i]->OnClosedConnection(systemAddress, rakNetGUID,
...
void TeamManager::OnNewConnection(const SystemAddress &systemAddress,
                                  RakNetGUID rakNetGUID, bool isIncoming) {
  for (unsigned int i = 0; i < worldsList.Size(); i++) {
...
void TeamManager::Send(const RakNet::BitStream *bitStream,
                       const AddressOrGUID systemIdentifier, bool broadcast) {
  SendUnified(bitStream, HIGH_PRIORITY, RELIABLE_ORDERED, 0, systemIdentifier,
              broadcast);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TeamManager::RemoveFromTeamsRequestedAndAddTeam(TM_TeamMember *teamMember,
                                                     TM_Team *team,
                                                     bool isTeamSwitch,
                                                     TM_Team *teamToLeave) {
  ...
  if (teamMember) {
    ...
    if (resultCode == 1) {
      ...
      if (packet->guid != GetMyGUIDUnified()) {
        ...
        bitStream.WriteCasted<MessageID>(ID_TEAM_BALANCER_TEAM_ASSIGNED);
        EncodeTeamAssigned(&bitStream, teamMember);
        SendUnified(&bitStream, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
                    packet->guid, false);
      }
    } else {
...
void TeamManager::OnUpdateTeamsRequestedToNoneAndAddTeam(Packet *packet,
                                                         TM_World *world) {
  BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(2 + sizeof(WorldId));
  NetworkID networkId;
...
void TeamManager::OnRemoveFromTeamsRequestedAndAddTeam(Packet *packet,
                                                       TM_World *world) {
  BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(2 + sizeof(WorldId));
  NetworkID networkId;
