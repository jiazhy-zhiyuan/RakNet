...
PluginReceiveResult FullyConnectedMesh2::OnReceive(Packet *packet) {
  switch (packet->data[0]) {
    ...
    case ID_NAT_CONNECTION_TO_TARGET_LOST: {
      ...
      b.Read(g);
      UpdateVerifiedJoinInProgressMember(g, UNASSIGNED_RAKNET_GUID,
                                         JIPS_FAILED);
      return RR_CONTINUE_PROCESSING;
    }
    case ID_NAT_PUNCHTHROUGH_FAILED:
      UpdateVerifiedJoinInProgressMember(packet->guid, UNASSIGNED_RAKNET_GUID,
                                         JIPS_FAILED);
      return RR_CONTINUE_PROCESSING;
  }
...
void FullyConnectedMesh2::OnClosedConnection(
  ...
  UpdateVerifiedJoinInProgressMember(rakNetGUID, UNASSIGNED_RAKNET_GUID,
                                     JIPS_FAILED);
...
void FullyConnectedMesh2::OnFailedConnectionAttempt(
  ...
  if (failedConnectionAttemptReason == FCAR_ALREADY_CONNECTED) {
    UpdateVerifiedJoinInProgressMember(packet->guid, packet->guid,
                                       JIPS_CONNECTED);
  } else {
    UpdateVerifiedJoinInProgressMember(packet->systemAddress,
                                       UNASSIGNED_RAKNET_GUID, JIPS_FAILED);
  }
}
  ...
  for (unsigned int i = 0; i < joinsInProgress.Size(); i++) {
    for (unsigned int j = 0; j < joinsInProgress[i]->vjipMembers.Size(); j++) {
      if (joinsInProgress[i]->vjipMembers[j].userData != 0) {
        RakNet::OP_DELETE(joinsInProgress[i]->vjipMembers[j].userData,
                          _FILE_AND_LINE_);
      }
    }
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(
    Packet *packet, bool accept, BitStream *additionalData) {
  ...
  CategorizeVJIP(&vjip, participatingMembersOnClientSucceeded,
...
void FullyConnectedMesh2::GetVerifiedJoinRejectedAdditionalData(
    Packet *packet, BitStream *additionalData) {
  RakNet::BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(sizeof(MessageID));
  if (additionalData) {
    additionalData->Reset();
...
void FullyConnectedMesh2::DecomposeJoinCapable(Packet *packet,
                                               VerifiedJoinInProgress *vjip) {
  RakNet::BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(sizeof(MessageID));
...
bool FullyConnectedMesh2::ProcessVerifiedJoinInProgressIfCompleted(
  ...
  WriteVJCUserData(&bsOut);
  SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, vjip->requester,
              false);
...
void FullyConnectedMesh2::WriteVerifiedJoinCapable(
    RakNet::BitStream *bsOut, VerifiedJoinInProgress *vjip) {
  bsOut->Write((MessageID)ID_FCM2_VERIFIED_JOIN_CAPABLE);
...
void FullyConnectedMesh2::ReadVerifiedJoinInProgressMember(
    RakNet::BitStream *bsIn, VerifiedJoinInProgressMember *vjipm) {
