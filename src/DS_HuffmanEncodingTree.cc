...
void HuffmanEncodingTree::GenerateFromFrequencyTable(
  ...
  int counter;
  HuffmanEncodingTreeNode *node;
  HuffmanEncodingTreeNode *leafList[256];  // Keep a copy of the pointers to all
                                           // the leaves so we can generate the
                                           // encryption table bottom-up, which
                                           // is easier
...
// 2.  While there is more than one tree, take the two smallest trees and merge
// them so that the two trees are the left and right children of a new node,
// where the new node has the weight the sum of the weight of the left and right
// child nodes.
#ifdef _MSC_VER
#pragma warning( \
  ...
  for (counter = 0; counter < 256; counter++) {
    ...
    do {
      if (currentNode->parent->left == currentNode)  // We're storing the paths
