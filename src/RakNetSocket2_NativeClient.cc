...
RNS2BindResult RNS2_NativeClient::Bind(
    NativeClientBindParameters *bindParameters, const char *file,
    unsigned int line) {
  ...
  if (Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4)) {
    ...
    RAKNET_DEBUG_PRINTF("CreateChromeSocket(%d,%s,0x%08x,%d) ==> 0x%08x\n",
                        bindParameters->port,
                        bindParameters->forceHostAddress
                            ? bindParameters->forceHostAddress
                            : "(null)",
    ...
    ((PPB_UDPSocket_Private_0_4 *)pp::Module::Get()->GetBrowserInterface(
         PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
        ->SetSocketFeature(rns2Socket, PP_UDPSOCKETFEATURE_BROADCAST,
                           PP_MakeBool(PP_TRUE));
    ...
    if (bindParameters->forceHostAddress) {
      ...
      if (bindParameters->is_ipv6) {
        ...
        NetAddressPrivate::CreateFromIPv4Address(ipv4, bindParameters->port,
                                                 &client_addr);
      }
    } else {
      NetAddressPrivate::GetAnyAddress(bindParameters->is_ipv6, &client_addr);
      NetAddressPrivate::ReplacePort(client_addr, bindParameters->port,
                                     &client_addr);
    }
...
void RNS2_NativeClient::SendImmediate(RNS2_SendParameters_NativeClient *sp) {
  ...
  NetAddressPrivate::CreateFromIPv6Address(
      sp->systemAddress.address.addr6.sin6_addr.u.Byte, 0,
  ...
  if (((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(
    ...
    if (family == PP_NETADDRESSFAMILY_IPV4) {
      ok = NetAddressPrivate::GetAddress(
          addr, &recvStruct->systemAddress.address.addr4.sin_addr,
          sizeof(in_addr));
    }
#if RAKNET_SUPPORT_IPV6 == 1
    else {
      ok = NetAddressPrivate::GetAddress(
          addr, &recvStruct->systemAddress.address.addr6.sin6_addr,
          sizeof(in6_addr));
    }
#endif
  }
