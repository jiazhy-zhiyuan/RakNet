...
HTTPConnection2::~HTTPConnection2() {}
bool HTTPConnection2::TransmitRequest(const char *stringToTransmit,
                                      const char *host, unsigned short port,
                                      bool useSSL, int ipVersion,
...
bool HTTPConnection2::GetResponse(RakString &stringTransmitted,
                                  RakString &hostTransmitted,
                                  RakString &responseReceived,
                                  SystemAddress &hostReceived,
                                  int &contentOffset) {
  void *userData;
  return GetResponse(stringTransmitted, hostTransmitted, responseReceived,
                     hostReceived, contentOffset, &userData);
}
bool HTTPConnection2::GetResponse(RakString &stringTransmitted,
                                  RakString &hostTransmitted,
                                  RakString &responseReceived,
...
void ReadChunkBlock(size_t &currentChunkSize, size_t &bytesReadSoFar,
                    char *txtIn, RakString &txtOut) {
  size_t bytesToRead;
  size_t sLen;
      ...
      */
      ...
      if (isFirstChunk) {
        ...
        if (sentRequest->thisChunkSize == 0) {
          ...
          body_header += 2;
          ReadChunkBlock(sentRequest->thisChunkSize,
                         sentRequest->bytesReadForThisChunk, body_header,
        ...
        ReadChunkBlock(sentRequest->thisChunkSize,
                       sentRequest->bytesReadForThisChunk, (char *)packet->data,
                       sentRequest->stringReceived);
        ...
        if (sentRequest->contentLength == -1) {
          ...
          if (length_header) {
            ...
            for (clLength = 0;
                 ...
                 clLength++)
              ;
            if (clLength > 0 && (length_header[clLength] == '\r' ||
...
void HTTPConnection2::OnNewConnection(const SystemAddress &systemAddress,
...
void HTTPConnection2::SendNextPendingRequest(void) {
  ...
  if (pendingRequests.Size() > 0) {
    ...
    if (pendingRequest->ipVersion != 6) {
      tcpInterface->Connect(pendingRequest->host.C_String(),
                            pendingRequest->port, false, AF_INET);
    } else {
#if RAKNET_SUPPORT_IPV6
      tcpInterface->Connect(pendingRequest->host.C_String(),
                            pendingRequest->port, false, AF_INET6);
#else
      RakAssert(
          "HTTPConnection2::TransmitRequest needs define  "
          "RAKNET_SUPPORT_IPV6" &&
  ...
  while (i < sentRequests.Size()) {
    if (sentRequests[i]->hostCompletedAddress == systemAddress) {
      ...
      if (sentRequest->chunked == false &&
        ...
        if (strstr(sentRequest->stringReceived.C_String(),
                   "Content-Length: ")) {
          char *body_header = strstr(
              (char *)sentRequest->stringReceived.C_String(), "\r\n\r\n");
          if (body_header) {
            body_header += 4;  // strlen("\r\n\r\n");
            sentRequest->contentOffset =
                body_header - sentRequest->stringReceived.C_String();
