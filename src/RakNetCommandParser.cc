...
RakNetCommandParser::RakNetCommandParser() {
  ...
  RegisterCommand(3, "CloseConnection",
                  "( const SystemAddress target, bool "
                  "sendDisconnectionNotification, unsigned char "
                  "orderingChannel=0 );");
...
bool RakNetCommandParser::OnCommand(const char *command, unsigned numParameters,
  ...
  if (strcmp(command, "Startup") == 0) {
    ...
    ReturnResult((int)peer->GetMaximumIncomingConnections(), command, transport,
                 systemAddress);
  } else if (strcmp(command, "Connect") == 0) {
    ReturnResult(
        peer->Connect(parameterList[0], (unsigned short)atoi(parameterList[1]),
