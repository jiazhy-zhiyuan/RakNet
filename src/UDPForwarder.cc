...
void UDPForwarder::RecvFrom(RakNet::TimeMS curTime,
...
#else
  receivedDataLen =
      recvfrom__(forwardEntry->socket, data, MAXIMUM_MTU_SIZE, flag,
    ...
    if (GetUsedForwardEntries() > maxForwardEntries) {
      ...
      if (sfos.result == UDPFORWARDER_RESULT_COUNT) {
        ...
        if (sfis->forceHostAddress.IsEmpty() ||
          ...
          getaddrinfo(sfis->forceHostAddress.C_String(), "0", &hints,
                      &servinfo);
