...
#if !defined(WINDOWS_STORE_RT)
bool TCPInterface::CreateListenSocket(unsigned short port,
...
bool TCPInterface::CreateListenSocket_WinStore8(
    unsigned short port, unsigned short maxIncomingConnections,
...
SystemAddress TCPInterface::HasCompletedConnectionAttempt(void) {
  ...
  if (sysAddr != UNASSIGNED_SYSTEM_ADDRESS) {
    unsigned int i;
    for (i = 0; i < messageHandlerList.Size(); i++)
      messageHandlerList[i]->OnNewConnection(sysAddr, UNASSIGNED_RAKNET_GUID,
                                             true);
  }
...
SystemAddress TCPInterface::HasNewIncomingConnection(void) {
  ...
  if (out) {
    ...
    unsigned int i;
    for (i = 0; i < messageHandlerList.Size(); i++)
      messageHandlerList[i]->OnNewConnection(out2, UNASSIGNED_RAKNET_GUID,
                                             true);
...
__TCPSOCKET__ TCPInterface::SocketConnect(const char *host,
            ...
            if (FD_ISSET(socketCopy, &readFD)) {
              ...
              if (len > 0) {
                ...
                */
                ...
                incomingMessage->deleteData = true;  // actually means came from
                                                     // SPSC, rather than
                                                     // AllocatePacket
