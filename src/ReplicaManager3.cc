  ...
  for (index2 = 0; index2 < replicaList.Size(); index2++) {
    ...
    if (action == RM3AOPC_DELETE_REPLICA) {
      ...
      for (unsigned int index3 = 0;
           index3 < connection->queryToSerializeReplicaList.Size(); index3++) {
...
void ReplicaManager3::DereferenceList(
    DataStructures::List<Replica3 *> &replicaListIn, WorldId worldId) {
  unsigned int index;
...
void ReplicaManager3::GetReplicasCreatedByMe(
    DataStructures::List<Replica3 *> &replicaListOut, WorldId worldId) {
  // RakNetGUID myGuid =
  // rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
...
Connection_RM3 *ReplicaManager3::GetConnectionAtIndex(unsigned index,
...
Connection_RM3 *ReplicaManager3::GetConnectionBySystemAddress(
    const SystemAddress &sa, WorldId worldId) const {
  RakAssert(worldsArray[worldId] != 0 && "World not in use");
  RM3World *world = worldsArray[worldId];
...
Connection_RM3 *ReplicaManager3::GetConnectionByGUID(RakNetGUID guid,
                                                     WorldId worldId) const {
  RakAssert(worldsArray[worldId] != 0 && "World not in use");
  RM3World *world = worldsArray[worldId];
  ...
  switch (packetIdentifier) {
    ...
    case ID_REPLICA_MANAGER_SERIALIZE:
      return OnSerialize(packet, packet->data, packet->length, packet->guid,
                         timestamp, packetDataOffset, incomingWorldId);
...
void Connection_RM3::SendConstruction(
  ...
  for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
    ...
    if (actuallyCreateObject) {
      ...
      bsOut.Write(offsetStart2);  // overwritten to point to after the call to
                                  // WriteAllocationID
      bsOut.AlignWriteToByteBoundary();  // Give the user an aligned bitStream
                                         // in case they use memcpy
  ...
  for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
    ...
    if (bsOut2.GetNumberOfBitsUsed() > 0) {
      ...
      offsetStart = bsOut.GetWriteOffset();
      bsOut.Write(
          offsetStart);  // overwritten to point to the end of the stream
      networkId = newObjects[newListIndex]->GetNetworkID();
      bsOut.Write(networkId);
      bsOut.AlignWriteToByteBoundary();  // Give the user an aligned bitStream
                                         // in case they use memcpy
  ...
  for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
    ...
    if (res != RM3SR_NEVER_SERIALIZE_FOR_THIS_CONNECTION &&
      ...
      SendSerialize(replica, allIndices, sp.outputBitstream,
                    sp.messageTimestamp, sp.pro, rakPeer, worldId, GetTime());
