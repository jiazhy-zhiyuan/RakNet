...
#if !defined(__GNUC__)
#pragma warning( \
    disable : 4309)  // 'initializing' : truncation of constant value
#endif
...
RakPeer::~RakPeer() {
  ...
  // Encryption and security
  CAT_AUDIT_PRINTF(
      "AUDIT: Deleting RakPeer security objects, handshake = %x, cookie jar = "
      "%x\n",
...
// SetMaximumIncomingConnections if you want to accept incoming connections
...
// set it to the sum of both types of connections \param[in] localPort The port
// to listen for connections on. \param[in] _threadSleepTimer How many ms to
// Sleep each internal update cycle. With new congestion control, the best
// results will be obtained by passing 10. \param[in] socketDescriptors An array
// of SocketDescriptor structures to force RakNet to listen on a particular IP
// address or port (or both).  Each SocketDescriptor will represent one unique
// socket.  Do not pass redundant structures.  To listen on a specific port, you
// can pass &socketDescriptor, 1SocketDescriptor(myPort,0); such as for a
...
// Pass 1 if you are not sure what to pass. \return False on failure (can't
// create socket or thread), true on success.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
StartupResult RakPeer::Startup(unsigned int maxConnections,
...
#if LIBCAT_SECURITY == 1
  CAT_AUDIT_PRINTF(
      "AUDIT: DisableSecurity() called, so deleting _server_handshake %x and "
      "cookie_jar %x\n",
      _server_handshake, _cookie_jar);
  RakNet::OP_DELETE(_server_handshake, _FILE_AND_LINE_);
  _server_handshake = 0;
...
// passwordData: A data block that must match the data block on the server.
// This can be just a password, or can be a stream of data passwordDataLength:
// The length in bytes of passwordData
//
// Returns:
...
// systemAddress specifies who not to send the packet to. Returns: \return 0 on
// bad input. Otherwise a number that identifies this message. If \a reliability
// is a type that returns a receipt, on a later call to Receive() you will get
// ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive
...
// of data \param[in] numParameters Length of the arrays data and lengths
...
// PacketPriority.h \param[in] orderingChannel When using ordered or sequenced
// messages, what channel to order these on. Messages are only ordered relative
// to other messages on the same stream \param[in] systemIdentifier Who to send
// this packet to, or in the case of broadcasting who not to send it to. Pass
// either a SystemAddress structure or a RakNetGUID structure. Use
...
// specifies who not to send the packet to. \return False if we are not
// connected to the specified recipient.  True otherwise
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#pragma warning(disable : 4701)  // warning C4701: local variable <variable
                                 // name> may be used without having been
                                 // initialized
#endif
  ...
  if (index < maximumNumberOfPeers)
    if (remoteSystemList[index].isActive &&
        remoteSystemList[index].connectMode ==
            RakPeer::RemoteSystemStruct::CONNECTED)  // Don't give the user
                                                     // players that aren't
                                                     // fully connected, since
                                                     // sends will fail
...
// \a guids[0] refer to the same system \param[out] addresses All system
// addresses. Size of the list is the number of connections. Size of the list
// will match the size of the \a guids list. \param[out] guids All guids. Size
// of the list is the number of connections. Size of the list will match the
...
/// reading it, or to override a value \param[in] systemAddress. The address to
/// set. Use SystemAddress::FromString() if you want to use a dotted string
...
// not being able to deliver a reliable packet \param[in] time Time, in MS
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
bool RakPeer::GenerateConnectionRequestChallenge(RequestedConnectionStruct *rcs,
  ...
  switch (publicKey->publicKeyMode) {
    ...
    case PKM_USE_TWO_WAY_AUTHENTICATION:
      ...
      CAT_AUDIT_PRINTF(
          "AUDIT: Success initializing new client handshake object with "
          "identity for this RequestedConnectionStruct -- pre-generated "
          "challenge\n");
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystemFromGUID(
    const RakNetGUID guid, bool onlyActive) const {
    ...
    if (doClientKey == 1) {
      ...
      if (_require_client_public_key) {
        ...
        if (!_server_handshake->VerifyInitiatorIdentity(
          ...
          RakNet::BitStream bitStream;
          bitStream.Write(
              (MessageID)ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY);  // Report an
                                                                 // error since
                                                                 // the client
                                                                 // is not
                                                                 // providing an
  ...
  *thisIPConnectedRecently = false;
  for (assignedIndex = 0; assignedIndex < maximumNumberOfPeers;
    ...
    if (remoteSystemList[assignedIndex].isActive == false) {
      ...
      remoteSystem->isActive = true;  // This one line causes future incoming
                                      // packets to go through the reliability
                                      // layer
      // Reserve this reliability layer for ourselves.
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::DeallocRNS2RecvStruct(RNS2RecvStruct *s, const char *file,
                                    unsigned int line) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RNS2RecvStruct *RakPeer::AllocRNS2RecvStruct(const char *file,
                                             unsigned int line) {
  bufferedPacketsFreePoolMutex.Lock();
...
void RakPeer::SendBufferedList(
  ...
  dataAggregate = (char *)rakMalloc_Ex(
      (size_t)totalLength, _FILE_AND_LINE_);  // Making a copy doesn't lose
                                              // efficiency because I tell the
                                              // reliability layer to use this
...
#else
    sendList = (unsigned *)rakMalloc_Ex(sizeof(unsigned) * maximumNumberOfPeers,
                                        _FILE_AND_LINE_);
#endif
...
void RakPeer::OnConnectedPong(RakNet::Time sendPingTime,
  ...
  RakNet::Time time =
      RakNet::GetTime();  // Update the time value to be accurate
  ...
  if (*isOfflineMessage) {
    ...
    if (((unsigned char)data[0] == ID_UNCONNECTED_PING_OPEN_CONNECTIONS ||
          ...
          rakPeer->AllowIncomingConnections())  // Open connections with players
      {
        ...
        outBitStream.Write(RakString((char *)rakPeer->offlinePingResponse.GetData()));
    ...
    // UNCONNECTED MESSAGE Pong with no data.
      ...
      rakPeer->AddPacketToProducer(packet);
               ...
               (size_t)length > sizeof(OFFLINE_MESSAGE_DATA_ID) +
        ...
        if (rcs->systemAddress == systemAddress) {
          if (serverHasSecurity) {
            ...
            if (cat::SecureEqual(public_key, rcs->remote_public_key,
              ...
              packet->data[0] = ID_PUBLIC_KEY_MISMATCH;  // Attempted a
                                                         // connection and
                                                         // couldn't
...
// Server does not need security
            ...
            if (rcs->client_handshake != 0) {
              ...
              packet = rakPeer->AllocPacket(sizeof(char), _FILE_AND_LINE_);
              packet->data[0] =
                  ID_OUR_SYSTEM_REQUIRES_SECURITY;  // Attempted a connection
                                                    // and couldn't
              packet->bitSize = (sizeof(char) * 8);
              packet->systemAddress = rcs->systemAddress;
              packet->guid = serverGuid;
...
// Move pointer from RequestedConnectionStruct to RemoteSystemStruct
              ...
              packet = rakPeer->AllocPacket(sizeof(char), _FILE_AND_LINE_);
              packet->data[0] = ID_CONNECTION_ATTEMPT_FAILED;  // Attempted a
                                                               // connection and
                                                               // couldn't
              packet->bitSize = (sizeof(char) * 8);
              packet->systemAddress = rcs->systemAddress;
      ...
      // TRUE,	  , FALSE	 , ID_ALREADY_CONNECTED (silently
      // disconnected, restarted rakNet) FALSE	  , FALSE	 , Allow
      // connection
...
// Send back the same answer
        ...
        if (requiresSecurityOfThisClient) {
          CAT_AUDIT_PRINTF(
              "AUDIT: Resending public key and answer from packetloss.  "
              "Sending ID_OPEN_CONNECTION_REPLY_2\n");
  ...
  if (remoteSystem) {
    ...
    if (isOfflineMessage == false) {
      remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer(
          ...
          remoteSystem->MTUSize, rakNetSocket, &rnr, timeRead, updateBitStream);
    }
  } else {
    // int a=5;
  ...
  while ((recvFromStruct = PopBufferedPacket()) != 0) {
    ...
    */
    ProcessNetworkPacket(recvFromStruct->systemAddress, recvFromStruct->data,
                         ...
                         recvFromStruct->socket, recvFromStruct->timeRead,
                         updateBitStream);
...
// SocketLayer::SetDoNotFragment(socketToUse, 0);
  ...
  // ++remoteSystemIndex )
  {
    ...
    remoteSystem->reliabilityLayer.Update(
        remoteSystem->rakNetSocket, systemAddress, remoteSystem->MTUSize,
