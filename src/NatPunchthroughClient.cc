...
PluginReceiveResult NatPunchthroughClient::OnReceive(Packet *packet) {
  switch (packet->data[0]) {
    ...
    case ID_NAT_RESPOND_BOUND_ADDRESSES: {
      ...
      for (int i = 0;
           i < boundAddressCount && i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) {
    ...
    case ID_OUT_OF_BAND_INTERNAL:
      if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
        ...
        if (natPunchthroughDebugInterface)
          natPunchthroughDebugInterface->OnClientMessage(
              RakString("HAS_PORT_STRIDE %i. First external port %i. Second "
                        "external port %i.",
        ...
        if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL &&
          ...
          if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
            ...
            if (natPunchthroughDebugInterface) {
              ...
              sp.targetGuid.ToString(guidString);
              natPunchthroughDebugInterface->OnClientMessage(
                  RakNet::RakString("PUNCHING_FIXED_PORT: Received "
                                    "ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid "
                                    "%s, system address %s.",
                                    guidString, ipAddressString));
            }
          } else {
            if (natPunchthroughDebugInterface) {
              ...
              sp.targetGuid.ToString(guidString);
              natPunchthroughDebugInterface->OnClientMessage(
                  RakNet::RakString("Received ID_NAT_ESTABLISH_UNIDIRECTIONAL "
          ...
          if (mostRecentExternalPort == 0) {
            ...
            if (natPunchthroughDebugInterface) {
              natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                  "ID_NAT_ESTABLISH_BIDIRECTIONAL mostRecentExternalPort first "
                  "time set to %i",
            ...
            if (sp.testMode != SendPing::TESTING_INTERNAL_IPS &&
              ...
              if (natPunchthroughDebugInterface) {
                natPunchthroughDebugInterface->OnClientMessage(
                    RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL: New "
                              "mostRecentExternalPort %i",
                              mostRecentExternalPort));
              }
            }
          ...
          if (natPunchthroughDebugInterface) {
            ...
            if (removedFromFailureQueue)
              natPunchthroughDebugInterface->OnClientMessage(
                  RakNet::RakString("Punchthrough to guid %s, system address "
                                    "%s succeeded on 2nd attempt.",
                                    guidString, ipAddressString));
            else
              natPunchthroughDebugInterface->OnClientMessage(
                  RakNet::RakString("Punchthrough to guid %s, system address "
                                    "%s succeeded on 1st attempt.",
                                    guidString, ipAddressString));
          }
        }
    ...
    case ID_NAT_TARGET_UNRESPONSIVE: {
      ...
      for (i = 0; i < failedAttemptList.Size(); i++) {
        if (failedAttemptList[i].guid == targetGuid) {
          ...
          // If the retry target is not connected, or loses connection, or is
          // not responsive, then previous failures cannot be retried.
      ...
      if (natPunchthroughDebugInterface) {
        ...
        targetGuid.ToString(guidString);
        natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
            "Punchthrough attempt to guid %s failed due to %s.", guidString,
            reason));
      }
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
  ...
  if (natPunchthroughDebugInterface) {
    ...
    if (oobId == ID_NAT_ESTABLISH_UNIDIRECTIONAL)
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("%I64d: %s: OOB ID_NAT_ESTABLISH_UNIDIRECTIONAL to "
                            "guid %s, system address %s.\n",
                            serverTime, TestModeToString(sp.testMode),
                            guidString, ipAddressString));
    else
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("%I64d: %s: OOB ID_NAT_ESTABLISH_BIDIRECTIONAL to "
                            "guid %s, system address %s.\n",
                            serverTime, TestModeToString(sp.testMode),
                            guidString, ipAddressString));
  }
...
*/
void NatPunchthroughClient::QueueOpenNAT(RakNetGUID destination,
                                         const SystemAddress &facilitator) {
  DSTAndFac daf;
