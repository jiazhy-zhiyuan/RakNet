...
RelayPluginEnums RelayPlugin::AddParticipantOnServer(const RakString &key,
...
void RelayPlugin::AddParticipantRequestFromClient(
    const RakString &key, const RakNetGUID &relayPluginServerGuid) {
  BitStream bsOut;
  ...
  SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, relayPluginServerGuid,
              false);
}
...
void RelayPlugin::SendToParticipant(const RakNetGUID &relayPluginServerGuid,
                                    const RakString &key, BitStream *bitStream,
  ...
  SendUnified(&bsOut, priority, reliability, orderingChannel,
              relayPluginServerGuid, false);
}
void RelayPlugin::SendGroupMessage(const RakNetGUID &relayPluginServerGuid,
                                   BitStream *bitStream,
                                   PacketPriority priority,
                                   PacketReliability reliability,
                                   char orderingChannel) {
...
RelayPlugin::RP_Group *RelayPlugin::JoinGroup(
    RP_Group *room, StrAndGuidAndRoom **strAndGuidSender) {
...
void RelayPlugin::JoinGroupRequest(const RakNetGUID &relayPluginServerGuid,
                                   RakString groupName) {
  BitStream bsOut;
  bsOut.WriteCasted<MessageID>(ID_RELAY_PLUGIN);
...
RelayPlugin::RP_Group *RelayPlugin::JoinGroup(RakNetGUID userGuid,
                                              RakString roomName) {
...
void RelayPlugin::NotifyUsersInRoom(RP_Group *room, int msg,
...
void RelayPlugin::SendMessageToRoom(StrAndGuidAndRoom **strAndGuidSender,
                                    BitStream *message) {
