...
BitStream::~BitStream() {
  if (copyData &&
      numberOfBitsAllocated > (BITSTREAM_STACK_ALLOCATION_SIZE << 3))
    rakFree_Ex(data, _FILE_AND_LINE_);  // Use realloc and free so we are more
                                        // efficient than delete and new for
                                        // resizing
}
  ...
  // do
  {
    ...
    if (numberOfBitsToWrite < 8 &&
        rightAlignedBits)  // rightAlignedBits means in the case of a partial
                           ...
                           // representation)
      dataByte <<= 8 - numberOfBitsToWrite;  // shift left to get the bits on
                                             // the left, as in our internal
                                             // representation
      ...
      *(data + (numberOfBitsUsed >> 3)) |=
      ...
      if (8 - (numberOfBitsUsedMod8) < 8 &&
          8 - (numberOfBitsUsedMod8) <
              numberOfBitsToWrite)  // If we didn't write it all out in the
                                    // first half (8 - (numberOfBitsUsed%8) is
                                    // the number we wrote in the first half)
      {
...
void BitStream::WriteCompressed(const unsigned char *inByteArray,
  ...
  while (currentByte > 0) {
    if (inByteArray[currentByte] == byteMatch)  // If high byte is byteMatch (0
                                                // of 0xff) then it would have
...
//	RakAssert( numberOfBitsToRead > 0 );
  ...
  while (numberOfBitsToRead > 0) {
    *(inOutByteArray + offset) |= *(data + (readOffset >> 3))
    ...
    if (readOffsetMod8 > 0 &&
        numberOfBitsToRead > 8 - (readOffsetMod8))  // If we have a second half,
                                                    // we didn't read enough
                                                    // bytes in the first half
...
bool BitStream::ReadCompressed(unsigned char *inOutByteArray,
  ...
  if (b)  // Check that bit
  {
    ...
    inOutByteArray[currentByte] |= halfByteMatch;  // We have to set the high 4
                                                   // bits since these are set
                                                   // to 0 by ReadBits
  } else {
