...
// TIME_BETWEEN_PACKETS_INCREASE_MULTIPLIER_DEFAULT=.02;  static const long double
// TIME_BETWEEN_PACKETS_DECREASE_MULTIPLIER_DEFAULT=1.0 / 9.0;
...
// total1+=value1; lastSec1+=value1;  total2+=value2; lastSec2+=value2;}  uint64_t
// BPSTracker::GetBPS2(RakNetTimeUS time) {ClearExpired2(time); return lastSec2;}
// void BPSTracker::GetBPS1And2(RakNetTimeUS time, uint64_t &out1, uint64_t
// &out2) {ClearExpired2(time); out1=lastSec1; out2=lastSec2;}
...
void ReliabilityLayer::Reset(bool resetVariables, int MTUSize,
  ...
  FreeMemory(true);  // true because making a memory reset pending in the update
                     // cycle causes resets after reconnects.  Instead, just
                     // call Reset from a single thread
  if (resetVariables) {
    InitializeVariables();
    ...
    for (i = 0; i < incomingNAKs.ranges.Size(); i++) {
      if (incomingNAKs.ranges[i].minIndex > incomingNAKs.ranges[i].maxIndex) {
        ...
        for (unsigned int messageHandlerIndex = 0;
             ...
             messageHandlerIndex++)
          messageHandlerList[messageHandlerIndex]
              ->OnReliabilityLayerNotification("incomingNAKs minIndex>maxIndex",
...
// Ack even unreliable messages for congestion control, just don't resend them
// on no ack
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS == 1
    ...
    while (internalPacket) {
...

      {
        ...
        if (internalPacket->reliability == RELIABLE ||
          ...
          holeCount = (DatagramSequenceNumberType)(
              internalPacket->reliableMessageNumber - receivedPacketsBaseIndex);
          ...
          } else  // holeCount>=receivedPackets.Size()
          {
            ...
            while ((unsigned int)(holeCount) > hasReceivedPacketQueue.Size())
              hasReceivedPacketQueue.Push(
                  true, _FILE_AND_LINE_);  // time+(CCTimeType)60 *
                                           // (CCTimeType)1000 *
                                           // (CCTimeType)1000); // Didn't get
                                           // this packet - set the time to give
                                           // up waiting
            ...
            RakAssert(hasReceivedPacketQueue.Size() <
                      (unsigned int)((DatagramSequenceNumberType)(
                          const uint32_t)(-1)));
#endif
          }
...
// Expected or highest known value
            ...
            for (unsigned int messageHandlerIndex = 0;
                 ...
                 messageHandlerIndex++)
              messageHandlerList[messageHandlerIndex]
                  ->OnReliabilityLayerNotification("Rejected older resend",
                                                   BYTES_TO_BITS(length),
                                                   systemAddress, false);
#endif
...
void ReliabilityLayer::Update(
    RakNetSocket2 *s, SystemAddress &systemAddress, int MTUSize,
...
// Due to thread vagarities and the way I store the time to avoid slow calls to
// RakNet::GetTime time may be less than lastAck
#if CC_TIME_TYPE_BYTES == 4
      ...
      while ((int)BITS_TO_BYTES(allDatagramSizesSoFar) <
        ...
        while (IsResendQueueEmpty() == false) {
          ...
          if (time - internalPacket->nextActionTime < (((CCTimeType)-1) / 2)) {
            ...
            // 						if (internalPacket->reliability==RELIABLE_ORDERED ||
            // internalPacket->reliability==RELIABLE_ORDERED_WITH_ACK_RECEIPT)
            // 							printf("RESEND reliableMessageNumber %i with datagram
            // %i\n", internalPacket->reliableMessageNumber.val,
...
*/
          ...
          // 					if (internalPacket->reliability==RELIABLE_ORDERED ||
          // internalPacket->reliability==RELIABLE_ORDERED_WITH_ACK_RECEIPT)
          // 						printf("SEND reliableMessageNumber %i in datagram
          // %i\n", internalPacket->reliableMessageNumber.val,
          // congestionManager.GetNextDatagramSequenceNumber().val);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendBitStream(RakNetSocket2 *s,
...
BitSize_t ReliabilityLayer::GetMessageHeaderLengthBits(
  ...
  if (internalPacket->reliability == RELIABLE ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT)
    bitLength +=
        8 * 3;  // bitStream->Write(internalPacket->reliableMessageNumber); //
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED ||
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
    bitLength += 8 * 3;  // bitStream->Write(internalPacket->orderingIndex); //
                         // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
                         // RELIABLE_ORDERED.
    bitLength += 8 * 1;  // tempChar=internalPacket->orderingChannel;
                         // bitStream->WriteAlignedVar8((const char*)&
                         // tempChar); // Used for UNRELIABLE_SEQUENCED,
                         // RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5 bits needed,
                         // write one byte
  }
  if (internalPacket->splitPacketCount > 0) {
...
BitSize_t ReliabilityLayer::WriteToBitStreamFromInternalPacket(
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      internalPacket->reliability == RELIABLE_SEQUENCED) {
    bitStream->Write(internalPacket->sequencingIndex);  // Used for
                                                        // UNRELIABLE_SEQUENCED,
                                                        // RELIABLE_SEQUENCED,
                                                        // RELIABLE_ORDERED.
  }
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED ||
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
    bitStream->Write(internalPacket->orderingIndex);  // Used for
                                                      // UNRELIABLE_SEQUENCED,
                                                      // RELIABLE_SEQUENCED,
                                                      // RELIABLE_ORDERED.
    tempChar = internalPacket->orderingChannel;
    bitStream->WriteAlignedVar8(
        (const char *)&tempChar);  // Used for UNRELIABLE_SEQUENCED,
                                   // RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5
                                   // bits needed, write one byte
  }
...
InternalPacket *ReliabilityLayer::CreateInternalPacketFromBitStream(
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      internalPacket->reliability == RELIABLE_SEQUENCED) {
    bitStream->Read(internalPacket->sequencingIndex);  // Used for
                                                       // UNRELIABLE_SEQUENCED,
                                                       // RELIABLE_SEQUENCED,
                                                       // RELIABLE_ORDERED.
  }
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED ||
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
    bitStream->Read(internalPacket->orderingIndex);  // Used for
                                                     // UNRELIABLE_SEQUENCED,
                                                     // RELIABLE_SEQUENCED,
                                                     // RELIABLE_ORDERED. 4
                                                     // bytes.
    readSuccess = bitStream->ReadAlignedVar8(
        (char *)&internalPacket->orderingChannel);  // Used for
                                                    // UNRELIABLE_SEQUENCED,
                                                    // RELIABLE_SEQUENCED,
                                                    // RELIABLE_ORDERED. 5 bits
  ...
  if (waitingForPacketOrderingIndex > maxRange / (OrderingIndexType)2) {
    if (newPacketOrderingIndex >= waitingForPacketOrderingIndex -
      ...
      return true;
               ...
               (OrderingIndexType)(
                   waitingForPacketOrderingIndex -
                   ((OrderingIndexType)maxRange / (OrderingIndexType)2 +
                    (OrderingIndexType)1)) ||
           newPacketOrderingIndex < waitingForPacketOrderingIndex) {
    return true;
  }
...
void ReliabilityLayer::SplitPacket(InternalPacket *internalPacket) {
  ...
  // with split packets
  internalPacket->splitPacketCount = 1;  // This causes
                                         // GetMessageHeaderLengthBits to
                                         // account for the split packet header
...
//-------------------------------------------------------------------------------------------------------
InternalPacket *ReliabilityLayer::BuildPacketFromSplitPacketList(
    SplitPacketIdType splitPacketId, CCTimeType time, RakNetSocket2 *s,
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendACKs(RakNetSocket2 *s, SystemAddress &systemAddress,
