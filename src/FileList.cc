  ...
  for (i = 0; i < fileList.Size(); i++) {
    ...
    if (fp == 0) {
      ...
      if (fileLength != fileList[i].fileLengthBytes &&
        ...
        //				fileData= (char*) rakMalloc_Ex( fileLength,
        //_FILE_AND_LINE_ );
        //				fread(fileData, fileLength, 1, fp);

        //				sha1.Reset();
        //				sha1.Update( ( unsigned char* ) fileData,
        //fileLength ); 				sha1.Final();
        ...
        if (fileLength != fileList[i].fileLengthBytes ||
          ...
          if (neverWriteHash == false)
            ...
            missingOrChangedFiles->AddFile(
                ...
                HASH_LENGTH, fileLength, FileListNodeContext(0, 0, 0, 0),
                false);
          else
            missingOrChangedFiles->AddFile(
                (const char *)fileList[i].filename,
...
#ifdef _MSC_VER
#pragma warning(disable : 4996)  // unlink declared deprecated by Microsoft in
                                 // order to make it harder to be cross
