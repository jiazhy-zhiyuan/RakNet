        ...
        switch (packet->data[1]) {
          ...
          case ID_UDP_PROXY_IN_PROGRESS: {
            ...
            if (packet->data[1] == ID_UDP_PROXY_FORWARDING_SUCCEEDED) {
              ...
              // Send a datagram to the proxy, so if we are behind a router,
              // that router adds an entry to the routing table. Otherwise the
              // router would block the incoming datagrams from source It
              // doesn't matter if the message actually arrives as long as it
              // goes through the router
              rakPeerInterface->Ping(serverIP.C_String(), forwardingPort,
                                     false);
          ...
          case ID_UDP_PROXY_ALL_SERVERS_BUSY:
            if (resultHandler)
              resultHandler->OnAllServersBusy(packet->systemAddress,
                                              senderAddress, targetAddress,
                                              targetGuid, this);
            break;
          case ID_UDP_PROXY_NO_SERVERS_ONLINE:
            if (resultHandler)
              resultHandler->OnNoServersOnline(packet->systemAddress,
                                               senderAddress, targetAddress,
                                               targetGuid, this);
            break;
          case ID_UDP_PROXY_RECIPIENT_GUID_NOT_CONNECTED_TO_COORDINATOR: {
