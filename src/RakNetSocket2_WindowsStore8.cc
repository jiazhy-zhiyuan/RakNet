...
ListenerContext::ListenerContext(Windows::Networking::Sockets::DatagramSocket ^
                                 listener) {
  this->listener = listener;
  //    InitializeCriticalSectionEx(&lock, 0, 0);
  outputStreamMap =
      ref new Platform::Collections::Map<uint64_t,
...
void ListenerContext::EchoMessage(DatagramSocketMessageReceivedEventArgs ^
                                  eventArguments) {}
...
RNS2SendResult RNS2_WindowsStore8::Send(RNS2_SendParameters *sendParameters,
...
int RNS2_WindowsStore8::WinRTInet_Addr(const char *str) {
  ...
  return htonl((parts[0] << 24) | (parts[1] << 16) | (parts[2] << 8) |
               (parts[3] << 0));
}

int RNS2_WindowsStore8::WinRTSetSockOpt(
    Windows::Networking::Sockets::DatagramSocket ^ s, int level, int optname,
...
int RNS2_WindowsStore8::WinRTIOCTLSocket(
    Windows::Networking::Sockets::DatagramSocket ^ s, long cmd,
    unsigned long *argp) {
...
int RNS2_WindowsStore8::WinRTGetSockName(
    Windows::Networking::Sockets::DatagramSocket ^ s, struct sockaddr *name,
    socklen_t *namelen) {
