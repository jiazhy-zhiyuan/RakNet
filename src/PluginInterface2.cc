...
void PluginInterface2::SendUnified(const RakNet::BitStream *bitStream,
  ...
  if (rakPeerInterface) {
    ...
    tcpInterface->Send((const char *)bitStream->GetData(),
...
void PluginInterface2::SendUnified(const char *data, const int length,
                                   PacketPriority priority,
                                   PacketReliability reliability,
                                   char orderingChannel,
                                   const AddressOrGUID systemIdentifier,
...
bool PluginInterface2::SendListUnified(const char **data, const int *lengths,
  ...
  if (rakPeerInterface) {
    ...
    return tcpInterface->SendList(data, (const unsigned int *)lengths,
                                  numParameters, systemIdentifier.systemAddress,
                                  broadcast);
  }
#endif
  ...
  if (broadcast == false && systemIdentifier.rakNetGuid == GetMyGUIDUnified()) {
    ...
    SendUnified(dataAggregate, totalLength, priority, reliability,
