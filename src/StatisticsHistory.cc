...
int StatisticsHistory::TrackedObjectComp(
    const uint64_t &key, StatisticsHistory::TrackedObject *const &data) {
...
StatisticsHistory::TrackedObjectData::TrackedObjectData(uint64_t _objectId,
                                                        int _objectType,
                                                        void *_userData) {
  objectId = _objectId;
...
bool StatisticsHistory::AddValueByObjectID(uint64_t objectId, RakString key,
                                           SHValueType val, Time curTime,
...
void StatisticsHistory::AddValueByIndex(unsigned int index, RakString key,
                                        SHValueType val, Time curTime,
                                        bool combineEqualTimes) {
...
StatisticsHistory::SHErrorCode StatisticsHistory::GetHistoryForKey(
    uint64_t objectId, RakString key,
    StatisticsHistory::TimeAndValueQueue **values, Time curTime) const {
...
bool StatisticsHistory::GetHistorySorted(
  ...
  DataStructures::OrderedList<TimeAndValueQueue *, TimeAndValueQueue *,
  ...
  for (unsigned int i = 0; i < itemList.Size(); i++) {
    ...
    if (sortType == SH_SORT_BY_RECENT_SUM_ASCENDING ||
      ...
      sortedQueues.Insert(tavq, tavq, false, _FILE_AND_LINE_,
                          TimeAndValueQueueCompAsc);
    else
      sortedQueues.Insert(tavq, tavq, false, _FILE_AND_LINE_,
                          TimeAndValueQueueCompDesc);
  }
...
void StatisticsHistory::MergeAllObjectsOnKey(
    RakString key, TimeAndValueQueue *tavqOutput,
  ...
  for (unsigned int idx = 0; idx < objects.Size(); idx++) {
    ...
    if (hi.IsInvalid() == false) {
      ...
      TimeAndValueQueue::MergeSets(tavqOutput, dataCategory, tavqInput,
                                   dataCategory, tavqOutput);
    }
  }
}
...
void StatisticsHistory::TimeAndValueQueue::MergeSets(
    const TimeAndValueQueue *lhs, SHDataCategory lhsDataCategory,
  ...
  if (timeClipEnd != 0 && histogram.Size() >= 1) {
    ...
    if (histogram.Peek().time > timeClipEnd) {
      histogram.Clear(_FILE_AND_LINE_);
               ...
               histogram.PeekTail().time > timeClipEnd) {
      StatisticsHistory::TimeAndValue tav;
...
SHValueType StatisticsHistory::TimeAndValueQueue::Interpolate(
    StatisticsHistory::TimeAndValue t1, StatisticsHistory::TimeAndValue t2,
    Time time) {
...
StatisticsHistory::TimeAndValueQueue &StatisticsHistory::TimeAndValueQueue::
operator=(const TimeAndValueQueue &input) {
  values = input.values;
...
void StatisticsHistoryPlugin::Update(void) {
  ...
  for (unsigned int idx = 0; idx < guids.Size(); idx++) {
    ...
    if (objectIndex != (unsigned int)-1) {
      ...
      statistics.AddValueByIndex(
          objectIndex, "RN_lastPing",
          (SHValueType)rakPeerInterface->GetLastPing(guids[idx]), curTime,
          false);
